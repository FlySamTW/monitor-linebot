...
#### 1. Fast Mode 誤用搜尋工具 → 帳單失控風險
- v24.5.6 時在 Fast Mode 啟用了 Google Search 工具
- Fast Mode 可以直接用網路補充，跳過 [AUTO_SEARCH_PDF] 機制
- 搜尋工具額外計費（雖然每次小額，但無控制感）
- **帳單風險**：如果 AI 習慣在 Fast Mode 先搜再用 QA，成本會「漸進式膨脹」

#### 2. 來源標註混亂
- 之前試了「[官方公告]」「[通用知識]」「[經驗建議]」等多種標籤
- Fast Mode 和 Deep Mode 的標籤邏輯不清晰，容易讓 AI 混淆
- 使用者無法判斷「這是從哪種來源拿到的答案」

### 解決方案

#### 原則：Fast Mode = QA Only；Deep Mode = PDF First + Optional Search

**Fast Mode（必須遵守）**
```
Fast Mode 禁用搜尋工具
  ↓
Fast Mode 只用 QA + CLASS_RULES 回答
  ↓
Fast Mode 找不到 → 輸出 [AUTO_SEARCH_PDF] 進 PDF
  ↓
所有回答末尾標「[來源: QA資料庫]」（簡化、單一責任）
```

**Deep Mode（允許靈活）**
```
Deep Mode 優先用 PDF 手冊回答
  ├─ 標「[來源: 產品手冊]」（佔 70-80% 回答）
  ├─ 若 PDF 不足，可選啟用搜尋工具補充官方資訊
  │  └─ 標「[來源: 網路搜尋]」（手冊已足夠就不要加）
  └─ 若 PDF 確定沒寫，加少量通用知識補充
     └─ 標「[來源: 非三星官方]」（非必要不加）
```
...
| 模式 | 場景 | 標籤 | 範例 |
|------|------|------|------|
| **Fast** | QA 或 CLASS_RULES 找到 | `[來源: QA資料庫]` | M8 有附視訊鏡頭 → [QA資料庫] |
| **Fast** | 無法回答 | `[AUTO_SEARCH_PDF]` | 怎麼設定 HDR → [AUTO_SEARCH_PDF] |
| **Deep** | PDF 直接有答案 | `[來源: 產品手冊]` | 設定 HDR 步驟 → [產品手冊] |
| **Deep** | PDF 有答案 + 搜尋補充 | `[來源: 產品手冊]` + `[來源: 網路搜尋]` | 設定 + 最新驅動資訊 |
| **Deep** | PDF 無答案 + 通用知識 | `[來源: 非三星官方]` | 「手冊未記載，以上憑經驗」 |

### 實施清單

**程式端修改（linebot.gs v24.5.8）**
- ✅ Fast Mode 動態提示：移除搜尋允許，明確禁止網路和非官方
- ✅ Deep Mode 動態提示：允許搜尋和非官方，但須必要判斷
- ✅ Google Search 工具改為僅在 PDF 模式掛載
- ✅ 版本號更新至 v24.5.8

**Prompt.csv 修改（v24.5.8）**
- ✅ 來源標註規則重新組織
- ✅ Fast Mode 嚴禁搜尋和非官方補充
- ✅ Deep Mode 可選搜尋，但必要才用
- ✅ 網路搜尋白名單（官方公告、韌體、驅動、安全、災情）
...
```
每日 1000 次對話
  └─ Fast Mode 800 次：約 50 次觸發搜尋（假設 6.25%）
  └─ Deep Mode 200 次：部分觸發搜尋
...
```
每日 1000 次對話
  └─ Fast Mode 800 次：全用 QA，不觸發搜尋 ✅
  └─ Deep Mode 200 次：智慧搜尋（必要才用）

搜尋成本完全隔離在 200 次 PDF 對話中
→ 成本可精確預測（200 次 × 平均搜尋率）
→ 預期節省 15-20%（無 Fast Mode 搜尋開銷）
```

### 防止帳單暴增的機制

1. **Fast Mode 壁壘森嚴**
   - 程式禁止搜尋工具：`tools` 只在 Deep Mode 設置
   - Prompt 明文禁止：「Fast Mode 嚴禁網路搜尋與非三星官方」
   - AI 無法繞過這兩層

2. **搜尋成本明確化**
   - 所有搜尋都在 Deep Mode
   - 搜尋所有搜尋都需 `[AUTO_SEARCH_PDF]` 觸發或故意掛 PDF
   - 可以手動審視「何時該搜、何時不該搜」

3. **Token 計費日誌**
   - 已有 `[Tokens] In: X, Out: Y` 日誌
   - 未來可按照 `[來源: 網路搜尋]` 標籤回溯成本

### 長期監控計畫

- 每週檢查「有 [來源: 網路搜尋] 的 Deep Mode 回答數」
- 與預期搜尋率（~20% Deep Mode）對比
- 若超過，調查 AI 為何傾向搜尋（Prompt 可能需調整）
