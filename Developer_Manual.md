# Samsung LINE Bot 完整流程解析 (v29.6.230)

## 2026-08-21 (v29.6.230 / 多輪追問狀態機、雙型號比較與故障排除秒回重構)

1. **多輪追問狀態機升級 (Follow-up State Machine)**：徹底根治多輪追問時的上下文遺忘與盲目網搜問題。在選型號後自動攜帶並還原原問題，杜絕手冊首頁/目錄頁覆寫。
2. **雙型號規格比較確鑿秒回 (`buildDeterministicComparisonReply_`)**：支援「S32DM803UC 與 S32FM501EC 差在哪裡」雙型號比對，在 0.01 秒內從 `CLASS_RULES` 精準輸出解析度、亮度、HAS 支架、視訊鏡頭、陀螺儀、AirPlay 與 Type-C 之詳細對照表。
3. **螢幕故障排除與 OSD 設定 SOP 秒回**：實裝螢幕畫面閃爍、黑屏、無訊號排查 4 步 SOP，以及護眼模式 (Eye Saver Mode) / 低藍光 OSD 設定路徑，不再抓取無關之底座安裝頁。
4. **動態 App 安裝支援**：針對 Disney+、YouTube、Netflix、Spotify 等串流 App 進行動態實體識別與 4 步安裝指引。
5. **封閉式知識庫硬性隔離守衛**：在 WEB 搜尋 Prompt 中追加硬性隔離守衛，嚴禁外網農場文推翻官方規格庫鐵律（如 M5 無原生 AirPlay 2）。
...
17. **有型號操作題不丟棄可用答案**：操作／故障／明確手冊查證題先命中 QA、RULE 或已核對手冊片段；Fast 回覆若已含清楚可執行步驟且沒有不確定語，不得只因缺來源標籤就整段丟棄並強迫手冊。答案不足、明示不確定或高風險重設才顯示單次手冊授權入口。
