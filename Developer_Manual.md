# Samsung LINE Bot 完整流程解析 (v29.6.157)
...
| R27 | `G8 有 KVM 嗎` → 選 `S32HG806ES` | 系列選型 → 精確型號 RULE 守門 | 選型不另扣；Fast 最多 1 次；PDF 0 | `術語_KVM` 不得當成該型號能力；完整規格列未記載時不能回答有／沒有，只能保留型號並建議手冊 |
...
- 精確型號守門結論不得被後段泛用手冊提示覆蓋；建議手冊的這一輪不得同時顯示 Web 搜尋入口，只有手冊仍無證據後才可推薦 Web。
...
10. **模型成本防呆**：Fast / Think / Polish 固定使用 `models/gemini-2.5-flash-lite`；只有 Web grounding 使用 `models/gemini-2.5-flash`。兩者都必須固定穩定版，不得使用 `latest` alias。
