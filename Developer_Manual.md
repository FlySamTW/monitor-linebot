# Samsung LINE Bot 完整流程解析 (v29.6.205)

## 2026-08-20 (v29.6.205 / 自動直通PDF、動態型號修剪網搜與全球中文解答)

1. **取消「手冊確認彈窗」多餘阻礙**：當 Fast Mode (QA/RULE) 查無資料，且系統鎖定或已帶有完整型號時，不再強制要求使用者手動點擊「📖 查官方手冊」按鈕，改為自動無縫直通進入 PDF／深度檢索流程。
2. **型號動態修剪多層級網搜 (Model Fuzzy Trimming)**：新增 `getSearchFriendlyModelTokens_` 函式。搜尋時若精準型號 (如 `S27FM501EC` / `LS27FM501ECXZW`) 在台灣網頁查無資料，自動依序修剪尾碼英文字與末位數字 (變 `S27FM50`、`M5`、`Smart Monitor M5`) 進行多層次擴展搜尋。
3. **解除語系與官網限制**：允許網搜檢索全球公開網頁資源，並強制要求 Gemini 將結果統一整理為完整、清晰的繁體中文解答，徹底杜絕無用廢話與降級失敗。
