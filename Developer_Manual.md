...
- v29.6.291 真人 TestUI 已完成 `G8 → S32DG802SC → PDF → Web`，但使用者可見文字仍出現「逐句核對引用」與 `；，`，屬內部術語及斷句殘留。
- 對外改成店員同儕說法：「我找到幾個非官方做法，但還不能確認完全適用這台，先試這三步」；連接詞移除後同步清掉前導標點。證據守門、模型與呼叫數不變，schema 升為 `EvidenceV9`。

## 2026-09-04（v29.6.291 / Google Search 後的非空白安全終點）

- 網搜的「找到候選內容」與「足以證實完整型號事實」分開判定。前者不能被誤報成沒搜尋，後者仍須 grounding support、來源與型號／主張驗證。
- 有逐句 grounding 但不足以證明 exact-model 時，只抽取支持句中的低風險操作；連逐句引用都不足時，才從本輪真實 Google Search 草稿抽取可逆排查步驟。包含推測、購買、其他型號、工程模式、韌體、產品能力、規格或數值的整句一律丟棄，不可拆句洗白。
- 沒有安全步驟時，依題型交付可現場驗證的方法、官網／三星客服或請 Sam 補 QA；同題仍由 operation cache 防止重搜與重扣。這是回答完成契約修正，不增加模型或供應商呼叫，schema 升為 `EvidenceV8`。

## 2026-09-04（v29.6.290 / Grounded Web 有用終點）

- 正式真人旅程證明 Google Search 已取得 YouTube／Reddit 的 grounding 與 PBP 操作句段，但舊 validator 因無法精確證明 `S32DG802SC` 型號能力，把整段丟棄成「網路不足」。這是證據降級策略過嚴，不是搜尋沒有結果。
- 新契約只從 `groundingSupports` 擷取低風險操作動作，移除型號能力、數值、規格、購買、韌體、工程模式與推測句；回覆明示「非官方、尚未確認完全適用本款」。未驗證的模型全文仍不可使用，精確產品事實仍須 exact-model 同來源證據。
- PDF→Web rescue 與直接 Web 共用相同降級機制。快取升為 `EvidenceV7`，避免同題重播 v29.6.289 的空白終點；模型與呼叫次數不變。

## 2026-09-04（v29.6.289 / 每則回覆顯示模型與完整費用）

- 正式 LINE 每一則回答最後都顯示單行摘要：`本次約 NT$…｜模型：…｜剩餘額度`；完全由 QA／RULE／程式回答時顯示 `未使用模型`。多泡泡回答從最後一個文字泡泡開始渲染，避免頁尾跑到回答中間。
- 模型名稱由 request audit 的實際成功生成紀錄產生，不由 LLM 自報：Router 與回答模型同輪出現時會列為 `Gemini 3.7 Flash（守門）＋Gemini 2.5 Flash`；Router 快取命中且本輪未呼叫時不計費、不冒充本輪模型。
- 費用使用供應商 `usageMetadata` 實際 input、candidate output、thinking tokens 計算；thinking 依官方規則併入 output 計費，多階段 Router／Fast／PDF／Web 全部累加。缺 usage metadata 時顯示「費用待確認」，禁止沿用上一題費用。
- 2026-09-04 Google 官方 Standard 價格複核：2.5 Flash-Lite 為 US$0.10／0.40（每百萬 input／含 thinking output tokens）；2.5 Flash 為 US$0.30／2.50；3.7 Flash 至 2026-12-31 優惠為 US$0.75／3.75，2027-01-01 起必須重新校價。台幣顯示以 `EXCHANGE_RATE=32` 換算，因此文字固定使用「約」。
- 2.5 Flash／Flash-Lite 的 Google Search grounding 目前共用每日 1,500 次免費額度，超出才為 US$35／1,000 grounded prompts；本專案 20 名實驗人員的 5 次網搜加系統救援上限仍遠低於此值。LINE 單題估算顯示 token 成本，不把尚未發生的超額搜尋費虛列進去。
- `G8／M8／M7` 等短稱若已由 CLASS_RULES 解析出系列與實際候選，視為確定性產品身分：系列共通規格用 RULE 交集回答，會因型號不同的操作直接列候選。兩者 `routerCalls=0`；Router 只處理句意、追問關係或複合主張的真正歧義，不重做系列對照。
- 一般問題已代表同意系統完成答案：QA／RULE／已驗證片段不足時，有適用手冊便自動查手冊；無適用手冊或前序來源仍未完成便自動查 Web，不再回「是否要搜尋」的空白交接。一般額度會在進階來源接管時退回，真正 Web 請求使用每日 5 次額度；PDF 失敗後的同次 Web rescue 仍使用獨立每日 3 次、不扣使用者 Web 額度。
- Web 不能保證外部世界一定有精確型號證據；工程契約是「一定到達有用終點」而非偽造答案。有可核對 grounding 就回答；只有相近資料、技術失敗或額度用完時，交付已確認部分、明確標示未證實的低風險排查方向與官網／請 Sam 補 QA，不顯示內部失敗或要求反覆重按。

## 2026-09-04（v29.6.288 / Router 最小啟動與帳單校準）

- 成本順序固定為 `精準 QA／RULE → 必要的 2.5 Flash-Lite 合成 → 2.5 Flash PDF／Web → 真正語意歧義才用 3.7 Router`；3.7 不得成為每題必經站。
- 已確認完整型號的操作／手冊題，以及明確的價格、庫存、活動等時效題，來源已可由程式決定，因此 `routerCalls=0`。只有型號／語意仍模糊、自然追問無既有規劃、複合多主張或來源規則真正衝突時，才可叫一次 3.7。
- 同一 canonical topic 已保存 Router claims 後，`那兩邊呢／那怎麼用` 等連續追問沿用既有規劃，`routerCalls=0`；明確換功能、換型號或提出新的複合題才視為新規劃需求。
- 2026-09-04 於 Google Cloud `Sam-Paid-Project` 實帳核對：2026-07 為 NT$46.18、2026-08 為 NT$21.95、2026-09-01～03 為 NT$1.24。帳戶報表使用 `$` 符號顯示台幣，已以 2.5 Flash token 單價交叉驗證，不得誤當美元。
...
- G9 後方燈效旅程的雲端 LOG 證明 PDF 確實執行了五次，並不是「沒有讀手冊」。真正缺陷是舊 Evidence 契約把共用手冊內標示「依型號而定，可能不支援」的 Core Lighting／Eclipse 段落誤當成 `S49DG952SC` 的正向證據，並容許 CoreSync、Core Lighting+、Infinity Core Lighting、Eclipse Lighting 等相近名稱互相代換，於是產生眼睛保護、OSD、Infinity Core、Eclipse 等不相干或無法證實的答案。
- 這是證據適用範圍、術語本體與狀態快取的結構性缺陷，不是 Gemini 2.5 Flash「看不懂 PDF」。v29.6.280 只把條件式語意規劃器提升為較強的 Gemini 3.7 Flash；真正回答產品事實的 Fast／PDF／Web 模型與來源順序都沒有改動。禁止再用升級整條回答鏈、堆 Prompt 或新增單題 if/regex 掩蓋 Evidence 問題。
- 本版的「根治」意義是結構性阻斷已知的錯型號、錯術語、條件式手冊誤採、跨站拼證據、舊 URI／舊結果重播與追問主題遺失；不應宣稱可以根除世界上所有幻覺。未取得足夠證據時寧可清楚保留未知，再依既定 PDF → Web 終點處理。

### 模型與固定路由

1. 指令、postback、配額、身分、精準 QA、完整 RULE 與人工核對 Evidence 先由程式處理；能完整回答便立即終止，`routerCalls=0`，不得因有 Router 而讓零成本題變慢或變貴。
2. 只有型號／系列模糊、複合多主張、省略式自然追問、QA／RULE 部分覆蓋或規則衝突時，才可呼叫一次 `models/gemini-3.7-flash` Router，設定 `thinkingLevel: low`。Router 使用 Structured Output、沒有搜尋／PDF／Web 等工具，也沒有 `answer` 欄位；它只能拆 claims、判斷追問關係及從程式候選 index 選型，不能產生產品事實、決定配額或覆蓋持久型號。
3. Fast 回答仍固定 `models/gemini-2.5-flash-lite`；整本 PDF 與 Web grounding 仍固定 `models/gemini-2.5-flash`。本版沒有把 PDF／Web 換成 3.7，也沒有新增背景搜尋、第二次潤飾或 Google File Search。
4. 程式仍是唯一來源決策者，順序固定為 `QA／RULE／verified Evidence → PDF → Web`。精準本機證據終止後不可再掛進階來源；已確認型號的未解操作／手冊型主張，不可被低信心 Router 送回 Fast 猜答案。
...
- 共用手冊只要相關段落含「依型號而定／可能不支援／部分型號」等適用性保留，且沒有同段正向綁定本題完整型號，就只能當候選線索，不能當該型號的正向 capability 或 operation 證據。具名功能也必須名稱一致，不准借另一個燈效功能代答。
- Web 證據以同一來源為原子單位：同一來源必須同時支持 canonical 型號身分與本題主張，才能形成肯定答案；不得拿網站 A 的型號身分加網站 B 的功能句跨站拼成一個結論。無可稽核引用時，不可把模型原始草稿或「大概可試」包裝成查證結果。
- 已確認完整型號與 canonical 主題跨日保留，讓「那要去哪裡開？」等省略追問仍承接正確產品與問題；只有使用者提出新完整型號或管理員執行 `/重啟` 才清除舊型號、主題與進階結果。新型號不得沿用舊型號的 PDF／Web 結果。
- 同一台北日期內，只有 `來源＋canonical 型號＋正規化主題＋Evidence schema＋來源指紋` 全部一致才可重用進階結果，命中時零新供應商呼叫、零再次扣次。PDF 指紋必須來自 Drive fileId、updatedAt、size／identity 等實際文件身分；Drive PDF 內容、時間或大小變更時，必須重新上傳並刷新 Gemini Files URI，舊 URI／舊答案不得續用。
- 來源成功、部分、無證據或失敗都必須到可預期終點；同一題不得因舊 operation cache 反覆重播錯答，也不得為相同主張連續執行五次 PDF。保留已有可信答案，只把未解 claim 交給下一來源。

## 2026-08-28（v29.6.279 / 競品範圍守門前移）

- v29.6.278 正式 TestUI 實問「LG 螢幕跟三星哪個比較好？」時，競品題因含「螢幕」而被一般螢幕情境過早放行，造成一次不必要的 Fast 呼叫，最後又要求店員按網路；這是共通分類順序錯誤，不是模型智力或單題 Prompt 問題。
- 競品螢幕辨識現已前移到一般配件與螢幕情境放行之前；跨裝置問題（例如 LG 手機接三星螢幕）仍沿用既有三星螢幕支援路徑。競品比較直接以同儕口吻安全收斂，`routerCalls=0 / fastCalls=0 / pdfCalls=0 / webCalls=0`，不再產生網搜 CTA。
- 這類資料邊界只靠 deterministic Scope Guard；不得往 Prompt 追加品牌單題特例，也不得用升級模型掩蓋分類順序。

## 2026-08-28（v29.6.278 / 店員同儕角色一致化）

- 清除 Fast 主提示中殘留的「台灣三星官方客服」身分與制式競品拒答範本，唯一角色改為門市店員的內部同儕助手。
- 時效資訊缺本機證據時只交由程式接續既有 Web 成本／Evidence 守門，不先回資料不足，也不再問店員是否要搜尋；未增加模型、搜尋工具或呼叫次數。
- 分類 Router 仍只做結構化分類；「零售模式」等手冊型用語由程式取得型號後直接查證，不把「手冊裡有沒有」反問店員。

## 2026-08-28（v29.6.277 / 條件式 RouteAnalysisV1 主張規劃器）

> **v29.6.277 當時契約（歷史）**：本節保留當時決策背景；若與最上方 v29.6.280 現行契約衝突，一律以 v29.6.280 為準。
...
- 正式 TestUI 的 `S57CG952NC 的 PBP 在哪裡開？` 已正確選到 `S49CG934,S57CG952.pdf`，手冊第 34 頁也確有 `PIP/PBP → PIP/PBP Mode`，但舊驗證器把回答開頭的型號數字 `57／952` 當成新規格數字，錯誤丟棄有效證據。這是全域 Evidence 驗證缺陷，不是缺檔，也不是為 PBP 增加答案特例。
- 數值守門改為先排除已由型號適用性契約另行驗證的完整型號／系列別稱，再核對功率、解析度、更新率等真正規格數字；因此正確的型號前綴可通過，無摘錄支持的 `120Hz` 仍會被拒絕。
- PDF Structured Output 先查目錄，再讀命中章節與相鄰頁；位置題必須輸出 `功能分類 → 設定項目`。Web 無引用補救改以完整句為單位過濾，不再因逗號切出「然後將其開啟」等殘句。
- 模型、thinking、供應商與費率完全不變：Fast 仍為 Gemini 2.5 Flash-Lite；整本 PDF／Web 仍為 Gemini 2.5 Flash、thinking=0；沒有啟用 Gemini 3.x、File Search 或第二次潤飾呼叫。

## 2026-08-27（v29.6.275 / 20 題全局 RAG 與追問契約）

- 本版先以固定 20 題真人常見問題（含自然追問、模糊系列、精確規格、操作、故障、比較與未知型號）跑正式 TestUI 基準；基準為完整 8、部分 5、失敗／不合理 7，Gemini 實測約 NT$1.2527。修復單位是共通資料與狀態契約，不為個別題目新增路由或 Prompt 特例。
- 回答鏈仍固定為 `產品身分 → QA → RULE → 已核對手冊片段 → 整本 PDF → Web 補救`。系列身分題若所有候選同屬一家族可直接回答；涉及型號差異才列完整型號。RULE 遇到 PBP／雙模等條件限定規格，不得拿整機最高值直接回答。
- 自動手冊預檢必須帶入已選完整型號；短自然追問會攜帶上一題主題。Fast 草稿若無可信證據且已有唯一型號與手冊，直接完成 PDF 查證，不再先回一個沒有答案的 CTA。
- 手冊結構化輸出新增 `coverage=full/partial/none` 與未解主張。入口題必須有實際選單路徑，複合題必須逐項覆蓋；只有相關背景不算完成。部分手冊證據會保留，再自動以公開網頁補足，不丟棄已確認內容。
- CLASS_RULES 的千分位逗號先在解析層正規化，仍維持 Google Sheet A 欄單一 CSV 字串；比較回答要提供可由已列規格推出的選擇方向。
- 目前正式 RAG 是 Gemini Files API：程式先依完整型號挑出一份 PDF，再以 `file_data` 送入 `generateContent`；不是 Google File Search store。File Search 不支援目前 2.5 模型，且需要升級模型與改造索引／API，本版沒有啟用，也沒有更換模型或費率。
- Files URI 採每 4 小時輪替續期 10 本，約百本可在 40 小時內更新一輪；每日 04:00 只做增量同步，避免全量重傳撞上 GAS 6 分鐘限制。真人查詢仍保留指定單檔續期與背景重試，不呼叫額外模型。
- 固定題庫在 `test_runner/datasets/v296274_web_common_questions_20.json`；回歸評估應分開檢查型號解析、檢索命中、證據覆蓋、答案品質、延遲與費用，不得看到一題失敗就加入該題字串特判。

## 2026-08-27（v29.6.274 / 結構化 QA 不得被別稱選型攔截）

- 正式 v29.6.273 真人驗收發現：`M8 怎麼安裝 Netflix` 已命中 `qa-smart-monitor-app-install`，但舊固定分數門檻仍拒絕分數 59.18 的答案，隨後才被系列別稱選型攔走。這不是缺型號，而是精準 QA 的可信條件過度僵硬。
- 修復採通用資料契約：候選必須有完整型號或系列別稱、至少兩個結構化意圖詞、分數至少 55，且領先第二名至少 8 分，才可在舊 68 分門檻下直接回答。任一條件不足仍維持 fail-closed，不猜答案。
- 路由順序不變：身分解析後先查精準 QA，再決定是否需要完整型號。沒有新增題目特例、Prompt 或模型呼叫；Fast／PDF／Web 模型與費率均不變。
- 正式 @1456 真人驗收：M8 Netflix 與 M7 零售模式皆零呼叫；S27DG5 自動解析、實讀正確 PDF 第 34 頁；G8 先選完整型號，手冊無明文後依契約完成一次 Web rescue。四條旅程皆有終點，新增費用共 NT$0.3386。
- Google 官方 2026-08-27 現價核對：2.5 Flash-Lite Standard 輸入／輸出 US$0.10／0.40；2.5 Flash 為 US$0.30／2.50（每百萬 token）。本版仍使用 Standard，沒有 3.x、Pro 或 Priority 額外費用。

## 2026-08-27（v29.6.273 / 產品身分必須先於回答與來源路由）

### 事故與決策原委

- 正式紀錄中，使用者先問過 `S49DG932SC`，再問 `Smart 如何開啟零售模式`。舊程式不認得句首 `Smart` 是 Smart Monitor 家族，因而把跨日保存的 S49 Odyssey 偷補進新題，接著操作題 early gate 又在 QA 前直接進手冊，最後掛錯 `S49DG932.pdf` 並做 Web 補救。這不是模型智力或 PDF 能力問題，而是產品身分與來源路由順序錯誤。
- 舊 `automaticFallback.priorFastChecked=true` 也只是宣稱已查過，沒有證明相同題目＋相同型號真的做過 QA／RULE／已核對片段預檢；任何新旁路都可能重現「QA 有答案卻直接讀 PDF」。因此不得再以布林旗標略過免費本機證據。
- 修復採通用身分契約，不新增「零售模式」等題目特例，也不把事故文字塞進 Prompt。解析順序固定為：本輪完整型號 → 本輪型號前段 → 本輪系列別稱 → 本輪具名家族 → 比較脈絡 → 跨日持久型號。新訊號與舊型號衝突時只暫停本輪沿用，不刪除跨日狀態；選定新完整型號才正式替換。

### 現行行為

- 句首 `Smart`／`Smart Monitor`／`智慧聯網螢幕` 會先視為家族。未給 M 系列時只問一次 `M5／M7／M8／M9`，回覆代號後接回原問題，不重複計入每日 20 題；不得借用舊 Odyssey。純問 Smart 系列有哪些型號仍可直接回答產品線。
- `M7` 等別稱若與已保存完整型號相容，直接沿用並回答；若目前保存的是 M8 或 Odyssey，便停用舊型號並從完整 CLASS_RULES 候選選型。`S27DG5`、`G806` 等前段先由 CLASS_RULES 收斂：唯一就直接帶入，多款才列按鈕。
- 型號候選完整集合保留最多 50 款，LINE 每頁只顯示 8 款並可上／下頁；不得再只保存畫面前 10 款。一般題候選來自 CLASS_RULES；真正手冊選型再與正式 PDF 索引取交集。
- 回答鏈固定為 `身分 → 精準 QA → 精確 RULE → 已核對手冊片段 → 選型 → 整本 PDF → 無證據才 Web 補救`。已移除 QA 前的 Operation Source Gate；自動進手冊仍會重新跑零成本本機預檢，避免旁路跳過答案。
- 成本分工不變：Fast `gemini-2.5-flash-lite`；整本 PDF／Web `gemini-2.5-flash`、thinking=0。未新增模型呼叫、第二次潤飾、搜尋或 Prompt 特例；`Prompt!C3` 及本機 Prompt 內容維持 v29.6.272。

### 防回歸案例

1. 先鎖 `S49DG932SC` 再問 `Smart 如何開啟零售模式`：只能問 M5/M7/M8/M9，`pdfCalls=0 / webCalls=0`。
2. 回覆 M7：保留原題並列完整 M7 候選；選 `S32FM703UC` 後以第 170 頁已核對片段零成本回答。
3. `M8 怎麼安裝 Netflix`：精準 QA 直接回答，不選型、不讀 PDF。
4. 已鎖 M7 後問 `M7 怎麼重設`：沿用原完整型號；已鎖 M8 後改問 M7：不得混用 M8。
5. `S27DG5 虛擬準心在哪`：前段唯一解析或列完整候選；大型 G5 候選可翻頁且後段型號不可遺失。
6. `/重啟` 後同題仍須依本輪 Smart 家族解析；代稱 `這台` 則維持跨日型號延續。

## 2026-08-23（v29.6.272 / 「怎麼選」不能只貼一張規格表）

- 雙型號比較仍只使用使用者指定的 RULE 欄位；當問句含「怎麼選／選哪／哪款適合」時，再從已列差異生成零模型選擇摘要。
- 摘要只做可機械核對的判斷：某款有明列支架、另一款未列；兩款對比值皆為 `數字:1` 時指出哪個官方數值較高。面板類型不自行判優劣，未列欄位也不補常識。
- 修圖比較範例應收尾為：在意支架調整可看 `S27H704EAC`；在意官方對比數值則 `S32D707EAC` 較高。仍為單次 deterministic、NT$0。
- 正式 v29.6.272 已用上述原句真人驗證為 deterministic、NT$0；同輪再驗證 `S32D806` 自然 KVM 追問，僅掛 1 份正確 PDF，取得第 17、19、21、33 頁與 `USB Source Setup／切換 USB 訊號源`，`pdfCalls=1 / webCalls=0 / thinking=0`。整輪測試約 NT$1.9696，模型、Prompt 與付費呼叫策略皆未提高。

## 2026-08-23（v29.6.271 / 模糊選型的年份也是 AND 條件）

- 使用者說年份時不能只比尺寸、解析度、面板與瓦數。Samsung 螢幕型號的世代碼統一解析為 T/A/B/C/D/F/H → 2020/2021/2022/2023/2024/2025/2026；RULE 若明載 `(2022)` 等年份，以明文優先。
- 年份與其他描述同為 AND 交集，零交集不猜。`M7＋32吋＋2025＋4K＋USB-C 65W` 只保留 `S32FM702UC／S32FM703UC`，不得混入 `S32BM702UC／S32BM703UC (2022)`。
- 兩款 2025 外觀版本仍同時符合時要讓使用者選，不因型號排序或新品偏好猜其中一款。此為全系列資料縮選規則，不是 M7 特例，也不呼叫模型。

## 2026-08-23（v29.6.270 / TestUI 重新整理必須等同 LINE 使用者再次開啟聊天室）

- `TEST_USER_ID` 已保存在 `sessionStorage`，但舊 `window.onload` 每次又呼叫 `clearTestSession()`，會刪除 Properties 中的產品型號、比較脈絡、來源狀態與當日配額；因此每次部署後重新整理都假裝成全新使用者，無法驗證跨日自然追問。
- 載入 TestUI 現在只重建畫面與讀取歷史，不再改寫伺服器狀態。只有測試者明確按「重置」或輸入管理指令 `/重啟` 才清除同一測試聊天室。
- 正式 LINE 路由不受這個前端缺陷影響；但正式 TestUI 是發布驗收工具，若它自動清狀態，會把真實追問誤報成程式回歸，並白花 Fast／PDF 測試費。

## 2026-08-23（v29.6.269 / PDF 省 token 不能把自然追問主題一起刪掉）

- PDF 仍只送一則訊息；只有本題含「這台／同一／跟著／前者／後者」等自然省略訊號時，才把上一個使用者問題壓成最多 180 字的一行主題，明示只供補全省略對象、不得重答前題。獨立新題不帶舊歷史。
- 手冊可能寫完整變體尾碼（如 `S32D806UAC`），而對話只鎖型號核心（`S32D806`）。只有 target 本身是裸核心、evidence 為該核心後接 1–4 個英文字母時視為同款；兩個完整型號仍不得靠前綴互套。
- 操作句裡提到 DP／Type-C 只是描述接法時，不再重貼整串 RULE。只有同句明問數量、瓦數、版本、規格或支援與否，才把對應已知規格併入手冊答案。
- 真人 S32D806 KVM 題證明手冊第 17 頁另有 `Switch USB`；舊版因前題 KVM 被裁掉且完整尾碼 evidence 被拒，只留下第 16、21 頁。修正以對話與身分契約處理，不新增 KVM 題型路由或模型呼叫。

## 2026-08-23（v29.6.268 / 手冊正確也不能把未問細節與 OCR 雜訊堆給使用者）

- 真人第 3 題已正確以 `S32FG502EC` 第 34 頁回答 `Game → Virtual Aim Point`，但又列出使用者未問的圖示樣式；其中圖示 OCR 成 `5:3、53、23、613`，屬於可核對卻不適合對外的雜訊。
- 對「在哪個選單／設定」的單純入口題，逐筆 evidence 驗證後再只保留入口或必要進入步驟，最多 2 筆；故障排除題不套此精簡，避免刪掉必要步驟。這是通用問題相關性規則，不是產品或功能特例。
- 不重查 PDF、不增加模型呼叫；v29.6.267 真人結果已證明模型、PDF、型號與頁碼正確，本版只做已驗證主張的零成本顯示篩選。

## 2026-08-23（v29.6.267 / PDF 每個回答主張必須各自通過證據驗證）

- PDF Structured Output 不再信任頂層自然答案或操作路徑；每筆 `evidence` 必須同時包含一個可直接給使用者的 `supportedAnswer`、頁碼、適用範圍與同頁摘錄。程式只組合逐筆通過的主張。
- 部分證據無效不得再整份丟棄：例如 2 筆適用目前型號、1 筆只適用其他型號時，保留前 2 筆並丟棄錯誤主張。只有 0 筆有效才進受控 Web 補救。
- 同頁可有多個不同主張；答案依主張去重，頁碼另行去重。摘錄外數字／單位、沒有摘錄直接支持的否定結論、錯型號、網址與內部標記都不得出站；`found=false` 卻帶 evidence 視為格式錯誤。
- 所有 `[MANUAL_*]` 內部狀態只供程式路由，LINE 只能看到自然說明。手冊失敗補救必須使用 Evidence Guard 後文字，不得把原始 marker 帶進 fallback。
- 模型、thinking、配額與供應商呼叫數不變：Fast 仍是 Gemini 2.5 Flash-Lite；PDF／Web 仍是 Gemini 2.5 Flash、`thinkingBudget=0`。本版只把同一次已取得的 PDF 結果安全轉成可用答案。

## 2026-08-23（v29.6.266 / 自然追問、來源授權與版本快取必須是同一型號）

- 比較後的「前者／後者」、或跨日已鎖定型號的自然追問，程式解析出的完整型號必須同時寫進 `msg`、`routingQuestion`、最近問題與來源 pending。任何舊候選 Cache 都不得在終答後把它覆寫；點手冊／網路時必須沿用本輪解析型號。
- 操作意圖以通用語法辨識；「哪個選單打開」「什麼設定開啟」「功能在哪裡切換」都應走既有 `QA/RULE → PDF → Web` 狀態機，不再先花 Fast 費用回一個來源按鈕。這是語序契約，不是虛擬準心題型特例。
- PDF／Web 同版本、同聊天室、同來源、同題、同型號仍在 10 分鐘內去重且不重扣；發布新版本代表回答或證據契約可能已修正，禁止跨版本重播上一版舊答案。否則正式修復部署後仍會看見舊錯答，真人驗收也會失真。
- 模型、thinking、配額與供應商呼叫規則不變：Fast 仍是 Gemini 2.5 Flash-Lite，PDF／Web 仍是 Gemini 2.5 Flash、`thinkingBudget=0`。

## 2026-08-23（v29.6.265 / 共用手冊的依型號證據不再自我矛盾）

- v29.6.264 正式真人題已正確掛載 `S32FG502EC` 共用手冊並找到第 49 頁，但 Structured Output 合約允許 `scope=依型號而異`，驗證器卻只接受「型號明確／全檔共通」，造成 1/3 證據通過時整份官方答案被丟棄並多做一次 Web。
- 驗證器現在接受三種 schema 合法 scope，但「依型號而異」仍必須由摘錄本身明載目前鎖定型號或去區域尾碼本體；只列其他型號、沒有型號限定或封面型號清單仍拒絕。因此修正是讓 schema 與驗證一致，不是放寬跨型號安全守門。
- 混合「全檔共通＋目前型號專屬」證據的終答一律標為「型號明確」，不能錯標整份手冊共通。新增正確目標、錯誤目標與混合 scope 契約。
- 不新增 Gemini／Web 呼叫、不換模型；目的正是讓已付費取得且可核對的 PDF 證據正常完成，避免不必要的第二次 Web 成本。

## 2026-08-23（v29.6.264 / 選型生命週期、比較完整性與自然追問）

### 為什麼這次不能再補單題特例

- 正式 TestUI 的同一聊天室連問暴露三個共用缺陷：系列候選先截前 10 款才比規格，導致後段新機永遠不出現；舊 `pending_topic / suggested_models` 沒有隨新問題結束，造成 S32D806、M7 等新題被上一輪 G5 選單綁架；比較函式只挑固定幾欄，使用者明問面板、對比、底座時仍可能只答解析度或更新率。
- 這些問題都屬於「型號狀態、證據欄位與問句」沒有共用契約，不是 Gemini 2.5 Flash-Lite 智力不足。若改 Prompt、升級模型或針對 G5／M7 加特例，只會增加費用並讓下一種問法再次回歸。

### 固定契約

- 系列別稱先取得完整 CLASS_RULES 候選（內部最多 50 款），再以首句明載的尺寸、解析度、更新率、面板、曲面／平面與供電瓦數做 AND 交集，最後才套畫面上限。零交集不猜；手冊模式再強制過濾為實際 PDF 索引已有覆蓋的型號，沒有 PDF 就不得把 RULE 型號冒充可查手冊。
- 型號選單、原題與模式一律同為 10 分鐘。點選的型號必須屬於目前候選；舊泡泡不得接回歷史舊題，也不得破壞目前題目的額度退款狀態。描述式補充只能縮小當前候選；帶有「怎麼、哪裡、幾瓦、KVM、RJ-45」等完整功能問句時視為新題，先清舊候選再走正常 QA／RULE → PDF → Web。
- 新問題不會刪除跨日正式產品記憶；只有新完整型號、有效選型或管理員 `/重啟` 才改變它。PDF 的主要型號以使用者實際選定值優先，不能讓共用手冊的推定型號覆蓋。
- 雙型號比較依使用者指定欄位逐項輸出；每一欄都必須得到兩款的 RULE 明文值或「官方規格未列此項」，不得局部回答後提前結束。比較狀態保留 30 分鐘：前者／後者或可唯一辨識的「32 吋那款」直接延續；只有「這台／它」仍不明確時才列原兩款讓使用者選，選定後接回原追問且不重複計次。
- Fast 仍為 Gemini 2.5 Flash-Lite；PDF／Web 仍為 Gemini 2.5 Flash、`thinkingBudget=0`。本版沒有換模型、增加來源呼叫或加入新的付費機制；新增的縮選、比較與狀態判斷皆為本機零模型邏輯。

### 必守回歸案例

- `G5＋27 吋＋QHD＋180Hz＋IPS` 必須保留 `S27DG502EC` 與 `S27FG502EC`，不得因前 10 款截斷漏掉新機，也不得猜成唯一一款。
- G5 候選後直接問 `S32D806 Type-C 幾瓦、RJ-45、KVM` 必須取消舊候選並回答 S32D806，不得沿用 G5。
- `S27H704EAC` 與 `S32D707EAC` 比較面板、對比、支架時要完整逐欄回答；接著問「前者／後者／32 吋那款」需鎖正確型號，問「這台」則只列這兩款。
- 手冊候選只來自實際 PDF 索引；合法按鍵、延遲舊按鍵、描述縮選、PDF 失敗補救與一般題退款必須各自維持單次、無迴圈契約。

## 2026-08-23（v29.6.263 / 共用手冊的證據不得跨型號套用）

- 十題真人旅程的 G8 MacBook 案實際掛載了正確的多型號共用 PDF，但第 12 頁同時有 HG802 與 HG806 的不同端子圖；模型將「只適用 S27HG802SC／S32HG802SC 的 USB-C 98W」誤套到 S32HG806ES。這是證據適用範圍缺口，不是 PDF 沒有讀取或模型太便宜。
- `normalizeManualStructuredResponse_()` 現在帶入鎖定完整型號。`scope=型號明確` 的摘錄必須含目前型號或其去區域尾碼本體；摘錄明載其他型號時一律失敗關閉，不得輸出產品事實。`scope=全檔共通` 只適用於真正全檔共通而且沒有衝突型號限定的原文。
- Structured Output 指令要求將圖、表或註記旁最近的適用型號連同原文一併摘錄；封面、目錄或「這本手冊包含此型號」不能補當功能證據。
- 已人工核對第 12 頁並新增兩筆資料層 `manual_chunk`：HG806 沒有 USB-C 影像／PD；HG802 具 USB-C 影像與最高 98W PD。日後同類知識以「型號＋意圖＋手冊頁碼」增加資料與契約測試，不新增題型路由或把特例塞進 Prompt。
- 不更換模型、不增加 Gemini／Web 呼叫；Fast 維持 Gemini 2.5 Flash-Lite，PDF／Web 維持 Gemini 2.5 Flash 且 `thinkingBudget=0`。
- 正式 TestUI 已重走「G8 → 32 吋 6K → MacBook 一線顯示加充電」：正確鎖定 `S32HG806ES`、接回原題並回答無 USB-C 影像／PD；以已核對手冊片段完成，`paidCalls=0 / pdfCalls=0 / webCalls=0`。

## 2026-08-23（v29.6.262 / 系列身分支援中英文與正反語序）

- 正式重測發現 Google 回的是「M8 智慧螢幕」，不是固定的「Smart Monitor M8」；系列與別稱順序不同仍代表同一個 RULE 身分。
- matcher 現在接受同一句 40 字內的 `Smart Monitor M8`／`M8 Smart Monitor`、`智慧螢幕 M8`／`M8 智慧螢幕`；仍要求完整型號自己的 RULE 明載 M8，不能跨到 Odyssey G8 或 M7。
- 只改文字身分正規化，沒有新增搜尋、模型呼叫或費用。

## 2026-08-23（v29.6.261 / Web 證據以完整型號或 RULE 系列身分核對）

- 非官方文章常只寫 `Smart Monitor M8`、`Odyssey G8` 等正式系列名，不會逐字列出台灣完整 SKU；舊驗證器因此會把已有 grounding 引用與可執行步驟全部丟掉。
- Web 身分驗證改為共用資料契約：完整 SKU／去地區尾碼型號可直接核對；系列別稱只能從目前完整型號自己的 CLASS_RULES 精確列取得，且必須帶正確產品家族語境。
- `S32FM803UC ↔ Smart Monitor M8` 可通過；`Odyssey G8`、`Smart Monitor M7` 或明說只找到其他系列仍拒絕。若系列只出現在完整搜尋回應而非支持句段，只限斷線等低風險排查，拆機、刷機及非官方韌體永不放行。
- 不新增 Gemini／Web 呼叫，不改模型、Prompt 本文、溫度、thinking、費率或額度；focus token、grounding support、完整句與危險操作守門全數保留。

## 2026-08-23（v29.6.260 / 完整型號操作題零 Fast 直進手冊）

- 操作意圖統一覆蓋「在哪、哪裡開、去哪、位置、怎麼開」等自然問法，不再因用字差異落入埠數規格回答。
- 精準 QA 與可完整回答的 RULE 均未命中，且已有完整型號時，選單、連接、故障等操作題零 Fast 直接進免費已驗證片段或完整 PDF。
- 純 VESA、HDMI 數量等規格題仍先由精確 RULE 零成本終止，不得因操作詞彙擴張而升級。

## 2026-08-23（v29.6.259 / 介面時序題不得被泛用片段截斷）

- 已驗證手冊片段只能回答它實際覆蓋的主張；「Dual Mode 如何開」的片段，不能替代「HDMI 是否能同時輸出 6K 165Hz」的訊號時序證據。
- 介面×解析度／更新率題在精準 QA 未命中後，跳過泛用手冊片段與 RULE，真正讀取完整 PDF。

## 2026-08-23（v29.6.258 / 介面訊號時序必須查手冊）

- 「有 HDMI 2.1」、「最高 6K」與「最高 165Hz」是三個獨立規格，不得拼成「HDMI 2.1 可同時跑滿 6K 165Hz」。
- 凡是指定 HDMI／DisplayPort／USB-C／Thunderbolt，再追問可達解析度或更新率的題目，精準 QA 未命中後立即查對應手冊的輸入訊號表。
- 這條路徑不先花 Fast 模型費用，也會退回「直接問」額度；不能只回端子數量而漏答時序問題。

## 2026-08-23（v29.6.257 / 一般與手冊候選共用描述選型）

- 「G8」等一般問答的型號候選，與「查官方手冊」候選共用同一個資料驅動解析器；不得另寫題型特例。
- 使用者回「32 吋 6K 那台」時，只在當前候選與 CLASS_RULES 唯一交集時選定型號，然後接回原問題；不得把這句當成新問題。
- 多款或零款符合時，保留原題與候選，請使用者再補一個特徵或點型號；不呼叫 LLM、不另外扣「直接問」次數。
- 一般問答候選可包含尚無 PDF 的型號，因為先走 QA／RULE；只有真正手冊候選才強制限定於 PDF 索引。

## 2026-08-23（v29.6.256 / 以自然描述完成手冊選型）

- 手冊候選已列出時，使用者可用「32 吋 6K 那台」「OLED 曲面那款」等自然描述繼續選型，不必知道完整型號。
- 系統只比對 pending 候選與其 CLASS_RULES 精確型號列，將尺寸、解析度、面板、平／曲面、更新率及雙模等明確條件做 AND 交集；只有唯一命中才自動選，不用模糊最高分猜測。
- 唯一命中後接回選型前的原問題；零命中或多款符合時保留原題並縮小／重列候選，選型階段維持零模型、零 PDF、零扣次。

## 2026-08-23（v29.6.255 / Gemini 思考 token 成本完整稽核）

- Google 官方 `usageMetadata` 會將模型內部思考量放在 `thoughtsTokenCount`；輸出成本必須以 `candidatesTokenCount + thoughtsTokenCount` 計算。官方費率依據：[Gemini API Pricing](https://ai.google.dev/gemini-api/docs/pricing)。
- 新增單一 `calculateGeminiUsageCost_()`，10 個既有 Gemini 計價點與 Request Audit 共用相同公式，並分開記錄回答、思考與實際計費輸出 token。
- `EXCHANGE_RATE = 32` 是顯示用估算；雲端帳單匯率、稅金及超過免費額度後的 Google Search grounding 費用不由本機公式推測。
- 不改 Gemini 2.5 Flash-Lite／2.5 Flash 分工、Prompt 本文、溫度、thinking 設定或模型呼叫次數，因此不會因本次修正增加 API 用量。

## 2026-08-23（v29.6.254 / 自然追問沿用型號與接頭證據）

- 正式 TestUI 四輪追問中，Fast 已正確回答「一般 HDMI 不能直接插 Micro HDMI」，但 `routingQuestion` 沒有重複完整型號，AnswerEnvelope 未使用已鎖定的 `S49DG932SC`，把正答當成無證據刪除後又要求查手冊／網路。
- 精確 RULE 入口現在會合併已鎖定 `primaryModel`；使用者不必每輪重打型號。支援的規格欄位會在 Fast／PDF 前零模型回答。
- HDMI 接頭關係採通用資料規則：確認實際 Micro HDMI 埠，並從同一型號配件欄核對 HDMI 轉 Micro HDMI 線；未明載時不得宣稱隨附。
- Prompt 本文、模型、溫度、配額與費率不變。

## 2026-08-23（v29.6.253 / 空 Quick Reply 不得吞掉答案）

- 正式 LINE 題目 `G932如何開啟PBP` 已正確選到 `S49DG932.pdf`，Gemini 6.06 秒產生第 35 頁答案；最後卻附上 `quickReply.items=[]`，LINE 回 400，造成使用者完全收不到。
- `buildAdvancedSourceQuickReplies_()` 沒有選項時改回傳空 options；`replyMessage()` 最後出口再檢查必須為非空陣列，雙層阻止任何來源送出空 Quick Reply。
- 已完成但未送達的同題 operation 可在原 10 分鐘 TTL 內跨相鄰 patch 沿用，避免修復部署後重查 PDF、再次扣費；逾時或非相鄰版本仍不得沿用。
- 模型、Prompt 本文、配額與費率不變。
...
10. **模型成本防呆**：Fast／Polish 固定 `models/gemini-2.5-flash-lite`，整本 PDF 與 Web grounding 固定 `models/gemini-2.5-flash`；只有符合條件的語意 Router 使用 `models/gemini-3.7-flash`、`thinkingLevel: low`、無工具與無答案欄位。全部使用明確穩定型號，不得使用 `latest`／`exp`，也不得把 3.7 擴散到每題回答或新增第二次潤飾。
