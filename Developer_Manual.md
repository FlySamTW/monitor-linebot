# Samsung LINE Bot 完整流程解析 (v29.6.092)

## 📋 核心哲學

本機優先自動長大 (QA/RULE First) + 官方手冊 RAG + 網路搜尋需註明 + 最後才用 AI 內建資料庫 + 程式防呆 (Code Guardrails)
...
- 事實型回覆只允許 `[來源:QA庫]`、`[來源:官方規格庫]`、`[來源:官方活動庫]`、`[來源:官方手冊]`、`[來源:網路搜尋]`、`[來源:AI內建資料庫]` 六種來源；其他標籤不得出現在 LINE 可見回覆。
...
- 範圍判斷看問題主體：手機、平板、電腦、遊戲機等裝置連接三星螢幕，仍是螢幕問題，必須照本機庫 → 官方手冊路由回答；只有純裝置問題才屬範圍外。若 LLM 仍誤拒答，系統只可保留相同來源重新呼叫 LLM，並把兩次費用合計顯示，不可拼固定答案。
- 跨裝置連接題若手冊只能確認螢幕端條件，必須保留手冊查到的內容並提示可點「這題再搜網路」查裝置端；完成 PDF 查詢後要同時留下快取與對話來源證據，後續明確網搜不得重跑 PDF。
- 官方手冊回答不得混入手冊未記載的手機設定、轉接器、上市狀態、換線測試等裝置端建議；若 LLM 越界，必須用同一本手冊重答並合計兩次費用，第二次仍越界才退回網路查證入口。
- 跨裝置 PDF 階段不得注入外部裝置相容性 QA；Samsung 官方手冊只負責螢幕端能力，Apple 等外部裝置能力必須留給具產品身分驗證的官方網搜。
- 「系統忙碌中」與其他 API 暫時失敗都屬終止狀態，不可再顯示「再詳細說明」或「這題再搜網路」。
- PDF 階段的既有 `isRetry` 只表示 Fast → PDF／Web 流程升級，不代表已做過證據修正；證據修正必須使用獨立旗標，否則自動升級 PDF 的回答會繞過來源守門。
- 跨裝置手冊回答固定採「一句結論＋最多 3 個螢幕端條件」；第二次 LLM 仍混入無來源裝置端建議時，只移除越界句子並保留 LLM 依手冊產生的內容，不可改塞題型固定答案。
- Google Search 只有同時取得 `groundingChunks` 與 `groundingSupports` 才算有可稽核網頁證據；缺任一者就重試一次，仍失敗則明說未取得來源，且不可標 `[來源:網路搜尋]`。
- 跨裝置連接題只提供 `M7`、`M8` 等短別稱時，必須在 Fast LLM 前直接進官方手冊型號選擇；提供完整型號時，Fast Mode 除非命中可信 QA，否則一律升級官方手冊，不能由規格庫推論手機相容性。
...
- PDF 手冊未記載外部裝置端能力時，不得斷言手機一定可連接、顯示、輸出或充電；這類肯定句與無來源設定建議同樣必須以同一本 PDF 重答，第二次仍越界才做句子級移除。網搜階段則可保留官方頁直接證據支持的裝置端結論。
...
18. **家電補型號不硬湊來源**：家電操作/維護題若需補完整型號，回覆是流程提示，不標來源；若背後有呼叫 LLM 仍必須標費用。
