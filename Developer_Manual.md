...
- previousQuestion + previousModel 是同一份最近題目狀態，保存 30 分鐘。手冊、網路完成後都必須繼續保存型號，禁止跨來源後只剩問題、型號歸零。
...
| R07 | 按「官方手冊」，已有上一題與型號 | MANUAL pending | 全部 0 | 顯示「查上一題／換型號／取消」與目前沿用型號 |
| R08 | R07 點「查上一題」 | 同題＋previousModel → MANUAL | 不扣一般；送出 PDF 前才扣手冊 1 | 型號直接組回查詢，不得重問或列全系列 |
...
| R16 | 同一題按「網路解答／再搜網路／查上一題」 | previousQuestion＋previousModel → WEB | 不扣一般；真正送出才扣網路 1 | 搜尋字串包含已鎖定完整型號，不先讀 PDF |
| R17 | 手冊 → 網路 → 再手冊，同一題 | 每階段保存同一問題與型號 | 各次只扣所選來源 1 | 網搜不得洗掉型號；再手冊不得重問型號 |
| R18 | 重新按同一來源 | 刷新 pending 10 分鐘 | 0 | 顯示最新剩餘額度與上一題；不得直接執行 |
...
- 「查上一題」與同題重打可沿用型號；不同題即使在 30 分鐘內，也不可因 Cache 有 last_selected_model 而偷借。
...
- 生產模型固定為穩定版 `models/gemini-2.5-flash-lite`。依 2026-08-15 Google 官方 Standard 價格，文字／PDF input 為 US$0.10／百萬 tokens、output（含 thinking）為 US$0.40／百萬 tokens；比 Gemini 3.5 Flash-Lite 的 US$0.30／US$2.50 更便宜，本次不得升級。
