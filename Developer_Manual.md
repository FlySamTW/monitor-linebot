...
- `不會再問一次` 屬內部流程解釋，對客戶沒有幫助且會讓人誤以為還有隱藏關卡；客戶文案只保留「想核對可點查官方手冊確認」。
...
4. **證據強度**：EXACT（精確 QA／RULE）、SUFFICIENT（已核對官方片段）可直接答；PARTIAL 可保留已知部分並只提供一次「查官方手冊確認」；NONE 才誠實要求缺少的完整型號或建議來源。證據較弱時，答案只能更保守，不能更肯定。
