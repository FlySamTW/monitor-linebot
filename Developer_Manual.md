# Samsung LINE Bot 完整流程解析 (v29.6.135)
...
`直接輸入 → SPEC/FAQ；按手冊 → 確認要查 → MANUAL；按網路 → 直接 WEB。每次只執行一個來源，完成後仍保留已確認型號。`
...
- 手冊選型契約：無完整型號時可用系列別稱或型號前段列候選；選型前不得讀 PDF、取得供應商授權或扣額度。手冊有上一題時顯示「確認要查／換型號／取消」；網路按鍵本身就是授權，不二次確認。

## 三來源使用者旅程與不可回歸矩陣（v29.6.135）
...
- 一般 20 題、手冊 5 次、網路 10 次是三個獨立額度。一次訊息最多只有一個付費階段，禁止雙重扣次或同訊息跨來源。
...
| R07 | 按「官方手冊」，已有上一題與型號 | MANUAL confirmation | 全部 0 | 顯示「確認要查／換型號／取消」與目前沿用型號 |
| R08 | R07 點「確認要查」 | 上一題＋持久型號 → MANUAL | 不扣一般；送出 PDF 前才扣手冊 1 | 型號直接組回查詢，不得重問或列全系列 |
...
| R11 | 手冊 pending 直接輸入不同問題 | 新題＋持久型號 → 確認 MANUAL | 不扣一般；確認前手冊 0 | 沒有新完整型號就沿用；新完整型號取代；短系列名列候選 |
| R12 | 手冊 pending 遇精準 QA／RULE 已足夠 | MANUAL 預檢 → SPEC/FAQ 回答 | 手冊 0；不重複扣一般 | 明示未讀手冊，回到 SPEC/FAQ |
| R13 | 型號無 PDF、URI 失效未修復、token／NT$0.35 守門擋下 | MANUAL 預檢失敗 | 手冊 0；PDF generate 0 | 不猜、不跨網路；說明未送出並提供下一步 |
| R14 | PDF 預檢通過 | MANUAL 單一來源 | 手冊 1；PDF 1；Web 0 | 最多 1 份 PDF（比較題 2 份），回答含檔名、頁碼、證據適用範圍與簡版費用 |
| R15 | 手冊只找到「依型號而定」或未找到直接證據 | MANUAL 結束 → WEB 建議 | 已送出則手冊 1；Web 0 | 不下肯定結論、不自動上網；須再由使用者按網路 |
| R16 | 按「網路解答／再搜網路」 | 題目＋持久型號 → WEB 直接執行 | 不扣一般；真正送出才扣網路 1 | 按鍵即授權，不二次確認；canonical query 含完整型號且不先讀 PDF |
| R17 | 手冊 → 網路 → 再手冊，同一題 | 每階段保存同一問題與型號 | 各次只扣所選來源 1 | 網搜不得洗掉型號；再手冊不得重問型號 |
| R18 | 10 分鐘內重送相同來源＋型號＋問題 | 語意操作快取 | 0 新供應商、0 新扣次 | 回傳前次結果；只有明確新問題才建立新操作 |
...
- 生產模型目前固定為 `models/gemini-2.5-flash-lite`。依 2026-08-15 Google 官方 Standard 價格，input 為 US$0.10／百萬 tokens、output 為 US$0.40／百萬 tokens；`gemini-3.5-flash-lite` 為 US$0.30／US$2.50。Google 明載 3.5 改善文件理解／解析，但不保證任一長 PDF 頁面必定命中；不可只因版本較新直接升級。
...
- 範圍判斷看問題主體：手機、平板、電腦、遊戲機等裝置連接三星螢幕，仍是螢幕問題，必須照本機庫 → 詢問同意 → 官方手冊路由回答；只有純裝置問題才屬範圍外。只有 429／5xx 可退避重試一次，所有費用只留在內部 LOG，不得顯示給客戶。
...
- 跨裝置 PDF 階段不得注入外部裝置相容性 QA；Samsung 官方手冊只負責螢幕端能力，Apple 等外部裝置能力必須留給具產品身分驗證的官方網搜。
...
- Google Search 只有同時取得 `groundingChunks` 與 `groundingSupports` 才算有可稽核網頁證據；缺任一者立即停止，不得為補引用自動再生成，也不可標 `[來源:網路搜尋]`。
...
- 網搜若能從問題推導出明確官方技術規格頁，應同時啟用 Google Search 與 URL Context；URL Context 只有回傳 `URL_RETRIEVAL_STATUS_SUCCESS` 才算證據，並把實際網址記入 LOG。搜尋 grounding 或官方 URL Context 任一成功才可顯示網搜來源。
- 跨裝置題完成官方手冊後再網搜時，必須把最近手冊回答當作螢幕端事實錨點；網搜只補外部裝置端資料，不得改寫手冊中的介面、線材條件、供電瓦數或限制。整合回答同時標 `[來源:官方手冊]` 與 `[來源:網路搜尋]`。
- 可推導出明確官方技術規格 URL 時，除 Search grounding／URL Context 外，應直接擷取官方頁文字作為證據；HTTP 200 且有足夠內容才算有效，實際網址與狀態寫入 LOG。官方頁已明載的能力不得回答成未明確標示。
- PDF 手冊未記載外部裝置端能力時，不得斷言手機一定可連接、顯示、輸出或充電；這類肯定句與無來源設定建議不得觸發同一本 PDF 的自動重答，改由下一輪明確網搜補足。網搜階段則可保留官方頁直接證據支持的裝置端結論。
...
2. **再官方手冊**：Fast Mode 不足只負責推薦；按手冊後仍先做免費 QA／RULE 預檢，未命中才顯示「確認要查」。確認後 PDF 生成階段只讀手冊；單次最壞 NT$0.35，超限先降解析度重算。單純選型不代表同意付費。已鎖定型號跨日沿用，直到新完整型號、換型號或管理員 `/重啟`。
3. **再網路搜尋**：價格/通路/活動/據點/最新資訊，或 PDF 明確無記載時，才引導網路搜尋，且網搜結果必須標 `[來源:網路搜尋]`。
4. **無證據就停止**：QA、CLASS_RULES、已授權手冊與已授權網搜都沒有證據時，直接說目前資料不足；具體產品、價格、設定、服務資訊不得用 AI 內建知識硬答，也不得標示 AI 內建資料庫。
...
10. **模型成本防呆**：Fast / Think / Polish 三個生產模型常數固定使用 `models/gemini-2.5-flash-lite`，不得使用 `latest` alias。
...
16. **無型號操作題不可泛猜**：操作/故障題沒有型號時，只有可信 QA 來源可直接回答；否則要回覆「請提供完整型號」，且流程提示不標來源。即使 AI 自行輸出 `[AUTO_SEARCH_PDF]`、`[AUTO_SEARCH_WEB]` 或 `[NEED_DOC]`，也不可越過補型號流程。
17. **有型號操作題仍需可信來源**：操作/故障/明確手冊查證題若有對應官方 PDF，Fast Mode 只有在命中 `[來源:QA庫]`、`[來源:官方規格庫]` 或 `[來源:官方活動庫]` 且答案品質足夠時才算完成；若沒有可信來源，即使 AI 產出看似完整的步驟，也必須升級官方手冊查證。
18. **流程提示不硬湊來源**：補完整型號、詢問是否查手冊與範圍外回覆都屬流程提示，不標來源；若背後有呼叫 LLM，費用只留在 LOG／TestUI 稽核，不顯示給客戶。
...
21. **API 失敗不外洩內部語句**：配額、逾時或供應商暫時失敗時，只能用「系統暫時忙碌，請稍後再試」這類客服語氣；不得叫一般 LINE 使用者升級付費方案，也不得假標來源。這類 API 失敗文案不得再被 `appendPdfSourceTag()` 補成官方手冊來源。
