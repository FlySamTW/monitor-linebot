...
> v29.6.281 現行額度與圖片覆蓋為：第二排 `10題/日`／`2次/日`／`5次/日`。下方若提及舊 20／5／10，只是版本沿革，不得拿來建立或發布 Rich Menu。
> v29.6.189 起，中鍵本身就是一次手冊授權；有上一題與型號就直接執行，缺型號只列實際 PDF 索引候選，選完直接查。手冊失敗會自動補查一次非三星公開網頁，不扣使用者 Web 額度。Quick Reply 由 AnswerEnvelope 依未解主張產生，手冊與 Web 可同時出現，不再互斥。
...
> v29.6.254 不變更 Rich Menu；只修正自然追問沿用已確認型號與 RULE 接頭／配件證據，避免正答被刪後再次顯示來源按鈕。
...
|---|---|---|---|---|
| 左 | `直接問` | `10題/日` | `rm_action=select_source&source=spec&v=2` | 清除來源 pending、保留型號、切回預設來源並開啟鍵盤 |
| 中 | `查手冊` | `2次/日` | `rm_action=select_source&source=manual&v=2` | 有目前問題與型號就立即查；缺型號只列實際 PDF 候選，缺題目才要求輸入 |
| 右 | `搜網路` | `5次/日` | `rm_action=select_source&source=web&v=2` | 有目前問題就直接搜尋；無問題才要求輸入 |
...
- 每位 userId 每日 10 次只在有效問題送出時原子保留；群組不共用。來源 postback、取消、型號提示與型號按鈕不得重複計次。手冊與網路分別為每日 2／5 次。
...
目前 `linebot.gs` 已有正式 Rich Menu postback fast path，且早於一般 message 與重型同步；後續修改仍須維持下列契約：
