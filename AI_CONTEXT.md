...
- `查官方手冊確認` 這一個按鍵即完成授權。已有「本題＋完整型號」就直接查 PDF；只有缺完整型號才列候選，選定後直接查；只有完全沒有題目才請使用者輸入題目。任何一路都不得再要求第二次確認。
...
- 無本機證據的 Fast 操作答覆只能作部分提示，並附一次「查官方手冊確認」；不得因步驟看似完整就當成已核對事實。已核對 evidence 命中才 terminal return。按鍵後直接查，不得二次確認。
