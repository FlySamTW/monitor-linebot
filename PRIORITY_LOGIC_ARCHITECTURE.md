...
⚠️ **重要**：本文檔記錄核心邏輯，修改前 **必須** 參考此文檔，避免重蹈 v24 版本的邏輯矛盾。
...
```markdown
### 修改場景：新增直通車關鍵字
- [ ] 確認新關鍵字是否真的需要強制 PDF（不是規格問題）
- [ ] 檢查 CLASS_RULES 中是否已有完整定義
- [ ] 測試「新關鍵字」的多個相關問題
  - [ ] 規格定義問題（無需進 PDF）
  - [ ] 操作步驟問題（需要進 PDF）
- [ ] 驗證 KEYWORD_MAP 映射是否正確（避免型號汙染）
- [ ] 確認不會在 Fast Mode 時跳過，必須先嘗試回答

### 修改場景：調整 PDF 匹配邏輯
- [ ] 確認 `hasInjectedModels` 仍然有作用
- [ ] 測試「同一對話中多次型號變化」的情況
  - 例如：「M8 怎麼樣」→「那 G8 呢」→「S32DG802SC 的話」
- [ ] 檢查日誌輸出的 PDF 數量是否合理
- [ ] 運行成本預算計算（預期 Token 數）

### 修改場景：新增 nonPdfPatterns
- [ ] 該模式是否屬於「規格定義」或「通識知識」
- [ ] 是否已在 Prompt 中明確指導 LLM「無需進 PDF」
- [ ] 測試該模式的 3-5 個相關問題
  - [ ] 確認都不進 PDF
  - [ ] 確認 Fast Mode 能回答

### 修改場景：調整搜尋工具限制
- [ ] 新限制是否與「官方信息不搜尋」的原則一致
- [ ] Prompt 中是否已有對應的指導
- [ ] 是否需要在 QA 中預先維護該信息
- [ ] 新限制是否會遺漏必要的補充信息

### 修改場景：新增/修改 QA
- [ ] 該 QA 是否比 PDF 的回答更清晰/更準確
- [ ] 是否是用戶重複問的問題（>3 次）
- [ ] 是否包含完整答案（不是「建議查 PDF」）
- [ ] 執行 `/重啟` 同步知識庫

### 修改場景：新增/修改 CLASS_RULES
- [ ] 新規格行是否遵循「型號：, 別稱1, 別稱2, ... 規格說明」格式
- [ ] LS 型號映射是否正確（LS27DG602SCXZW → S27DG602SC）
- [ ] 新術語是否有歧義（不是只有三星有）
- [ ] 執行 `/重啟` 同步知識庫
- [ ] 驗證 KEYWORD_MAP 中的映射（檢查 LOG）

### 修改場景：調整優先級層次
⚠️ **極度危險**，每個改動都可能破壞邏輯

- [ ] 先閱讀本文檔的「優先級架構」部分，確保理解
- [ ] 識別該改動影響的所有 3-5 個觸發點
- [ ] 寫下「為什麼要改」和「改了會怎樣」
- [ ] 設計完整的 test case（至少 20 個不同問題）
- [ ] 在 Prompt 中也做相應更新
- [ ] 在 log 中新增診斷 debug 信息
- [ ] 等待一周，觀察日誌中是否有異常
```
...
| 問題類型 | 模式 | Token 數 | 成本 |
|---------|------|---------|------|
| 「什麼是 HDR」 | Fast | 3-5K | $0.0005 |
| 「M8 有視訊鏡頭嗎」 | Fast | 2-3K | $0.0001 |
| 「M8 怎麼開啟 HDR」 | Deep (1 PDF) | 60-70K | $0.65 |
| 「M8 和 M7 怎麼比較」 | Deep (2 PDF) | 100-120K | $1.2 |
| 「所有 M 系列的規格」 | Deep (多本) | 150K+ | $1.5+ |

### 日誌關鍵詞查詢

```javascript
// 檢查是否正確進 Fast Mode
[KB Load] AttachPDFs: false

// 檢查是否發生型號汙染
[KB Select] 命中型號: S32FM803, M80D, M70D...  // ❌ 有汙染

// 檢查直通車是否被記錄（不立即開 PDF）
[Direct Search] 命中直通車關鍵字: M8，先走 Fast Mode

// 檢查 [AUTO_SEARCH_PDF] 是否被觸發
[Auto Search] 偵測到搜尋暗號

// 檢查是否正確進 Deep Mode
[KB Load] AttachPDFs: true, Files: 1 / 15

// 檢查是否正確被 nonPdfPatterns 攔截
[Non-PDF Q] 通識/規格定義問題，不進 PDF
```
