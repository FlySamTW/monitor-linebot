...
9. **模型成本防呆**：Fast / Think / Polish 三個生產模型常數固定使用 `models/gemini-2.5-flash-lite`；主流程與維護端點不得硬寫其他 Gemini 模型、`latest` alias 或 `exp` alias；`batchTest`、`testRun`、`testModels` 這類會觸發 LLM 的 doGet 維護端點必須先驗證密鑰；`LLM_PROVIDER` 與 OpenRouter 模型若要變更，必須先做成本審查並更新靜態守門。
...
15. **型號泡泡前導不可用 AI 中間稿**：顯示型號選擇時，前導文字必須是固定流程提示並帶 `[來源:專案流程規則]`；不可把 Fast Mode 的未驗證回答一起送給使用者。
...
18. **無型號操作題不可泛猜**：操作/故障題沒有型號時，只有可信 QA 來源可直接回答；否則要回覆「請提供完整型號」，並標註 `[來源:專案流程規則]`。即使 AI 自行輸出 `[AUTO_SEARCH_PDF]`、`[AUTO_SEARCH_WEB]` 或 `[NEED_DOC]`，也不可越過補型號流程。
19. **有型號操作題仍需可信來源**：操作/故障/明確手冊查證題若有對應官方 PDF，Fast Mode 只有在命中 `[來源:QA]` 或 `[來源:規格庫]` 且答案品質足夠時才算完成；若沒有可信來源，即使 AI 產出看似完整的步驟，也必須升級官方手冊查證。
20. **家電補型號也要標來源**：家電操作/維護題若需補完整型號，回覆同樣必須標註 `[來源:專案流程規則]`，不可變成無來源客服建議。
21. **Fast Mode 來源精確白名單**：只有 LLM 明確輸出 `[來源:QA]` 或 `[來源:規格庫]` 才能補回來源；模糊來源如 `[來源:QA資料庫]`、`[來源:產品規格表]` 一律丟棄，避免把未命中的內容洗成可信來源。
...
28. **不存在完整型號早期攔截**：若使用者輸入看似完整型號（如 Sxx/LSxx/WA/WD/VR/G90XF 類型），但 QA/CLASS_RULES/PDF 型號索引都找不到，必須在 LLM 前先回覆請確認型號並標註 `[來源:專案型號驗證規則]`；不得進 LLM 猜規格，也不得假標 QA、規格庫或手冊來源。
