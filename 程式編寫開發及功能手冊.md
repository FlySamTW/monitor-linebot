...
- `deploy.bat` 會依序推送程式、建立版本、更新既有 Webhook；部署流程**不會**修改 Google Sheet `Prompt!C3`，避免本地鏡像誤覆蓋正式 Prompt。
...
- **Ops**: 新增受保護的 `update_prompt_c3` 維護入口與 `tools/sync_prompt_c3.ps1`，供明確維護 Prompt 時使用；日常部署不得自動覆蓋 Google Sheet `Prompt!C3`。
