...
- 新 SKU 每日自動發現，但只進 `PENDING_MODEL_REVIEW`；第一次正式 RULE 仍需人工核准。
- 新手冊只從台灣 Samsung support model 頁與官方 download center 的繁中 `UM` 取得；檢查官方 host、`UNI_TW／UM`、PDF 檔頭、大小與 SHA-256，每輪最多 2 本。
- 自動下載只進 Drive `_PENDING_MANUAL_REVIEW` 子資料夾，正式 RAG 不讀；人工核對第一頁型號、共用範圍與繁中後才依命名規則移入根目錄。既有手冊 hash 改變也先 staging，舊版繼續服務。
...
- 2026-08-15 正式索引當次盤點：H／2026 世代 6 款 RULE 型號全部有 PDF 覆蓋，缺口 0。未來可自動發現 Samsung 支援頁下載網址，但正式入庫前仍必須驗證第一頁完整型號、MIME／PDF magic、SHA-256 與檔名；驗證不足就停在人工審核，不得直接污染 RAG。
