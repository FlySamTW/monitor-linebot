# Samsung LINE Bot 完整流程解析 (v29.5.264)
...
10. **型號選擇回覆要乾淨**：任何要求使用者先選型號或補型號的回覆，不得追加查手冊提醒；等使用者選定型號後，再依 QA/規格 → PDF → WEB 流程處理。
11. **API 失敗不外洩內部語句**：配額、逾時或供應商暫時失敗時，只能用「系統暫時忙碌，請稍後再試」這類客服語氣；不得叫一般 LINE 使用者升級付費方案，也不得假標來源。
12. **版本上限處理**：若 Apps Script 達 200 版本上限，先到 Project History 批次刪除未被 active deployment 使用的舊版本，再重跑 `deploy.bat`；不可因此新建正式 deployment ID。可先執行 `tools/check_deploy_readiness.ps1` 確認正式 Webhook 版本、版本數與阻塞原因。
...
- 必須使用既有 Deployment ID 更新部署，除非使用者明確要求新建部署。
- 健康檢查應回傳最新 `GAS_VERSION` 才算正式 Webhook 生效。
- `deploy.bat` 會依序推送程式、建立版本、更新既有 Webhook；若本機已設定 `GAS_ADMIN_SECRET`，最後會自動把 `Prompt.csv` 同步到 Google Sheet `Prompt!C3`。

### Prompt 同步

- 正式 Prompt 位於 Google Sheet `Prompt` 工作表的 `C3`。
- `Prompt.csv` 是本地鏡像與人工貼回來源，不是正式執行來源。
- 修改 Prompt 後必須同步到 `Prompt!C3`，再 `/重啟` 或重建快取確認重啟訊息中的「指令版本」。
- Prompt 同步使用受 `GEMINI_API_KEY` 保護的 `update_prompt_c3` 維護入口。
- 本機可用 `tools/sync_prompt_c3.ps1` 同步；執行前需把環境變數 `GAS_ADMIN_SECRET` 設成 GAS Script Properties 內的 `GEMINI_API_KEY`。
...
- **Ops (Deploy Readiness)**: 新增 `tools/check_deploy_readiness.ps1`，可快速比對本機版本、正式 Webhook 健康檢查、GAS 版本數與 active deployments。
...
- **Ops**: 新增受保護的 `update_prompt_c3` 維護入口與 `tools/sync_prompt_c3.ps1`，讓 `Prompt.csv` 可同步到 Google Sheet `Prompt!C3`。
- **Ops**: `deploy.bat` 改為四步驟：推送程式、建立版本、更新既有 Webhook、可選同步 Prompt!C3。
