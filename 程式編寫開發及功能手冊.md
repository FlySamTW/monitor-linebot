...
- 一般訊息固定走規格／FAQ；完整型號跨日保存。官方手冊需「確認要查」，網路按鍵本身即授權；每次只允許一個進階來源。
...
2. **再官方手冊**：Fast Mode 不足只負責推薦；按手冊後先做免費 QA／RULE 預檢，未命中才顯示「確認要查」。確認後 PDF 生成階段只讀手冊；單次最壞 NT$0.35，超限先降解析度重算。手冊／網路各用自己的額度，不重複扣一般 20 題；已鎖定型號跨日沿用，直到新完整型號、換型號或管理員 `/重啟`。
...
10. **模型成本防呆**：Fast / Think / Polish 三個生產模型常數固定使用 `models/gemini-2.5-flash-lite`；主流程與維護端點不得硬寫其他 Gemini 模型、`latest` alias 或 `exp` alias；`batchTest`、`testRun`、`testModels` 這類會觸發 LLM 的 doGet 維護端點必須先驗證密鑰；`LLM_PROVIDER` 與 OpenRouter 模型若要變更，必須先做成本審查並更新靜態守門。
...
19. **有型號操作題仍需可信來源**：操作/故障/明確手冊查證題若有對應官方 PDF，Fast Mode 只有在命中 `[來源:QA庫]`、`[來源:官方規格庫]` 或 `[來源:官方活動庫]` 且答案品質足夠時才算完成；若沒有可信來源，即使 AI 產出看似完整的步驟，也必須升級官方手冊查證。
...
31. **TestUI 與維運端點必須授權且不可污染正式資料**：`?test=1` 與 LOG、RULE、PDF、同步等維運端點必須使用 `MAINTENANCE_SECRET` 或 `OPENCODE_WRITE_SECRET`；不得把 Gemini API 金鑰當通行碼。測試模式的 QA/RULE 草稿只能預覽，不得寫入正式 Sheet。
