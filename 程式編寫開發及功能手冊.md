...
- Fast／PDF／Polish 固定為 `models/gemini-2.5-flash-lite`（Standard US$0.10／US$0.40 每百萬 tokens）；只有 Web grounding 使用穩定版 `models/gemini-2.5-flash`（US$0.30／US$2.50）。此例外只為修復 Flash-Lite 不執行搜尋工具，不得擴到 QA／RULE／PDF。
