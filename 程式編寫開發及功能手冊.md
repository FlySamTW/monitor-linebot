# Samsung LINE Bot 完整流程解析 (v29.5.129)
...
| 按鈕 | text | 顯示條件 | 處理方式 |
|------|------|----------|----------|
| 💬 再詳細說明 | `#再詳細說明` | 永遠 | 改寫 msg 後**不 return**，走正常對話流程 |
| 📖 查PDF手冊 | `#查手冊` | `hasPdfForModel=true` | 獨立 handler，從歷史找問題 → PDF → LLM |
...
對照 PDF_MODEL_INDEX (ScriptProperties)
  ↓
有 PDF → hasPdfForModel = true → 顯示「📖 查PDF手冊」按鈕
