# Samsung LINE Bot 完整流程解析 (v29.6.035)
...
9. **模型成本防呆**：Fast / Think / Polish 三個生產模型常數固定使用 `models/gemini-2.5-flash-lite`；主流程與維護端點不得硬寫其他 Gemini 模型、`latest` alias 或 `exp` alias；`LLM_PROVIDER` 與 OpenRouter 模型若要變更，必須先做成本審查並更新靜態守門。
