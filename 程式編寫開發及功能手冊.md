...
- 一般訊息固定走規格／FAQ；官方手冊與網路解答只能由本輪 postback 或相容指令取得 10 分鐘一次性授權。
...
- 手冊模式可接受系列別稱或型號前段；多個版本時用 `select_manual_model` postback 讓使用者點選。新題不得借用上一題型號，選型前也不得讀 PDF 或扣次。
...
2. **再官方手冊**：只有 Fast Mode 不足且使用者明確授權，並完成必要型號選擇後，才掛載 PDF RAG。免費 QA／RULE 預檢未命中後，PDF 生成階段只讀手冊；單次最壞 NT$0.35，超限先降解析度重算。手冊／網路各用自己的額度，不重複扣一般 20 題；只有點「查上一題」才沿用已鎖定型號。
