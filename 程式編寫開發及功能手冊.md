# Samsung LINE Bot 完整流程解析 (v29.5.243)
...
- 若使用一次性維護入口同步 Prompt，完成後必須立刻移除入口並重新部署，避免留下公開寫入路徑。
...
- 若目前雲端 ScriptProperties 已被舊版本歸零，只能等下一次 Gemini File API 成功上傳/同步後恢復；新版本保證不再把可用索引覆蓋成 0。
