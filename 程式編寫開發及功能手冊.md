# Samsung LINE Bot 完整流程解析 (v29.5.270)
...
15. **API 失敗不外洩內部語句**：配額、逾時或供應商暫時失敗時，只能用「系統暫時忙碌，請稍後再試」這類客服語氣；不得叫一般 LINE 使用者升級付費方案，也不得假標來源。這類 API 失敗文案不得再被 `appendPdfSourceTag()` 補成官方手冊來源。
16. **版本上限處理**：若 Apps Script 達 200 版本上限，先到 Project History 批次刪除未被 active deployment 使用的舊版本，再重跑 `deploy.bat`；不可因此新建正式 deployment ID。可先執行 `tools/check_deploy_readiness.ps1` 確認正式 Webhook 版本、版本數與阻塞原因。
