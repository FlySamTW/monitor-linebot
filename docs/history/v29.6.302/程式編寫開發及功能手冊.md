...
- v29.6.301 真人 LOG 已正確鎖定 S32DG802SC、命中手冊第 101 頁並扣 1 次，但頁級模型仍回 `partial`，導致多跑約 NT$0.0842 的 Web；整題約 NT$0.0880。這是完成狀態誤判，不是檢索失敗。
- 三方人工同義成立、題目為單一選單路徑且 evidence 有直接入口時，程式直接判 full；複合、數值、限制、非操作或證據不完整均不得套用。
- 回覆把 OCR 方向鍵與選單文字整理成人能直接照做的步驟。預期只使用一次 2.5 Flash-Lite，`routerCalls=0 / pdfCalls=1 / webCalls=0`，不增加模型或搜尋階段。

## 2026-09-05（v29.6.301 / 頁級 RAG 配額對齊）

- 頁級 RAG 是一次實際 Flash-Lite 生成，必須與整本 PDF 共用原子配額保留。v29.6.299 曾因 fast path 繞過 `callLLMWithRetry()` 而誤顯示未送出與剩餘 2/2。
- `callManualPageRag_()` 現在於供應商請求前執行 `reserveAdvancedSourceUsage_()`；沒額度就不送，送出就計一次，不增加任何模型或額外請求。

## 2026-09-05（v29.6.300 / RULE 支持的手冊同義完成）

- v29.6.299 已正確召回 S32DG802SC 手冊第 101 頁，但因 RULE 的 PBP 與手冊 UI 的「多重視窗」名稱不同，完成度守門誤判 partial，白跑 Web 後並追加「沒有足夠證據」。這不是 retrieval 或模型失敗，而是同義完成判定過度保守。
- 現改為三方強證據：原題用詞、精確型號官方 RULE、官方手冊名稱必須同時命中人工核定 lexicon 群，且該群明示允許，純名稱差異才可 full。少任一錨點、或仍缺數值／限制／多主張，仍要 partial 並補查。
- 頁級完整答案改為自然文字，不顯示「已確認規格／手冊補充」程式階段；同題只要一次 2.5 Flash-Lite 頁級整理，不叫 Router、不附整本 PDF、不叫 Web。

## 2026-09-05（v29.6.299 / 型號綁定頁級 RAG）

- v29.6.298 的 medium 整本 PDF 正式實測仍花約 NT$0.5155／43 秒且漏掉答案頁，證明瓶頸在 retrieval，不在 Router 或解析度。官方 S32DG802SC 支援手冊的可核對入口是 PDF 第 101 頁「設定 → 多重視窗」。
- 新手冊順序為 `唯一型號 registry → 本機 BM25 → curated alias／家族守門 → 最多三段官方原文 → 2.5 Flash-Lite 整理`。模型只能選 evidenceId，頁碼、摘錄、PDF SHA 與型號綁定由程式回填。
- BM25 最近頁不代表有答案；頁面沒有功能 canonical alias 就不建立群組，未命中才回整本 PDF／Web。這是全域證據門檻，不是 PBP 特例。
- 整本 PDF ceiling 撤回 NT$0.35；medium 超標才免費測 low，實際生成最多一次。Router 的 3.7 low 條件完全不擴大，精準 QA／RULE、已解析系列、已確認型號單一操作與來源按鍵仍為零 Router。
- Google managed File Search 能代管 chunk、index 與 citation，但 current API 偏 3.x；legacy 只列 2.5 Flash-Lite，不列整本 PDF 用的 2.5 Flash。File Search 與 Search grounding 不可同請求並用，2.5 也無法同時 File Search＋Structured Output，所以只保留為日後同題 A/B 候選，不在本版換模型或改 API。

## 2026-09-05（v29.6.298 / PDF medium 實讀與 Web 型號隔離）

- v29.6.297 已證明系列選型、完整型號鎖定、正確 PDF allowlist 與 Router skip 可正常工作；答案仍失敗是因 244 頁 PDF 被 NT$0.35 ceiling 降成 low，漏掉實際第 101 頁，不是 Router 不夠聰明。
- 第一次 `countTokens` 前即設定 `MEDIA_RESOLUTION_MEDIUM`，單次最壞 ceiling 調為 NT$0.60；medium 超標才再測一次 low，生成仍只有一次。這只影響已授權的整本 PDF fallback，不影響 QA／RULE 快路。
- Web tentative evidence 必須在同一 source-set 綁定 exact model。別代 G8、其他型號或「例如／像是」列出的介面不再回給目前型號；未綁 citation 的草稿不可提供型號特定選單路徑。
- 模型不變：Fast 2.5 Flash-Lite、PDF／Web 2.5 Flash、條件式 Router 3.7 Flash low。Router 不擴大到每題，也沒有新增第二次潤飾或搜尋。
- 業界與 Google 官方建議是把 retrieval、groundedness、answer relevance、延遲與有效答案成本分開評估；Structured Output 仍需應用端驗證。managed File Search 可另案 A/B，但 current／legacy 支援表均未列現行 PDF 用的 2.5 Flash，不在此版遷移。

## 2026-09-05（v29.6.297 / PDF medium-first 與 File Search 再評估）

- 低解析度整本 PDF 可能找到錯頁，即使 Evidence Guard 能避免錯答，仍會浪費一次查詢。超標 PDF 現在先以 medium 免費預檢，只有 medium 仍超過 NT$0.35 才降 low；真正生成次數不變。
- Google 最新 File Search 已支援 legacy `generateContent`、chunk citation 與 PDF page number，且 storage／query embedding 免費；但現行 PDF 模型 2.5 Flash 不在支援表。遷移必須另案用同一手冊、同一題庫 A/B，不得為使用新技術直接換模型。
- RAG 驗收分開看召回頁、證據 groundedness、答案完成度、延遲與合計成本；不能只看有沒有回文字。

## 2026-09-05（v29.6.296 / QA 召回的 scope 與 intent 分層）

- QA 排序必須先用完整型號／系列／家族做適用範圍過濾，再以功能詞判斷題意；兩種訊號不可相加後當成同一種相關性。
- `strongSignal` 至少需要一個非身分、非泛用動詞的實質詞。產品身分加上「開啟、設定、安裝、顯示」不能命中；實質功能詞再搭配操作詞且排名明顯領先，才可精準零成本回答。
- 直答與 Prompt context 使用同一 gate，避免錯誤 QA 從另一條路注入。失配後只回既有 RULE／PDF／Web 流程，不增加 Router 或回答模型呼叫。

## 2026-09-04（v29.6.295 / RULE 能力與手冊操作分層）

- 精確型號操作題拆為能力 claim 與操作 claim：能力只取 QA／RULE，操作只取 PDF 頁面。`buildKnownRuleAnchorForMixedOperation_()` 依 CLASS_RULES ontology 動態保留能力，不新增單題特例。
- 手冊沒有原題名稱但有可完成相同使用目的的入口時，只能明標「手冊中可查到的相近操作」，不可說兩功能等同，並強制 partial 後只搜未解主張；數值與模式限制不適用此降級。
- 快取升為 `EvidenceV10`。Router／Fast／PDF／Web 模型、額度及供應商呼叫上限不變。

## 2026-09-04（v29.6.294 / 操作題 Evidence 完成度）

- 正式 TestUI 實例證明：整本 PDF 已有直接啟動路徑，模型仍可能只擷到「啟動後」控制而誤判完成。往後不只驗證有頁碼與關鍵字，還必須驗證原始問題要的 answer shape。
- 「怎麼開、如何連接、哪個選單」統一要求入口路徑或具體選擇動作；只有後續控制者拒絕。PBP、PIP、Multi View 各自綁證據，禁止相似功能代答。
- PDF 提示改為優先摘錄「若要啟動／前往」入口句；Web 操作題只回入口與 2–4 步、320 字內。不增加模型、呼叫次數或額度。


## 2026-09-04（v29.6.293 / 條件式 Router 準確性與成本收旂）

- confirmed model 的單一操作／故障題、RULE 已解析的系列別稱、精準 QA／RULE、明確時效題與來源按鍵均不啟動 Router。舊候選快取不得讓已確認型號重新變模糊。
- Router 回傳後增加通用語意不變式：已確認型號強制沿用；時效資訊強制 Web；未有完整本機證據的操作／故障強制手冊；無 previousTopic 的短句不得冒充追問。未增加單題 regex。
- 保留 `gemini-3.7-flash` low 作必要歧義分類，不在沒有 A/B 證據時降級模型。以官方現價重算，常見 Router 約 NT$0.025–0.048，單次超過 NT$0.10 記錄警示；原 NT$0.003–0.006 是舊 Flash-Lite 估算，不可沿用。
- 契約題庫增至至少 20 種啟動／略過情境，覆蓋已確認型號夾舊候選、G8 系列、單一排障、時效題、部分證據、複合題與追問承接。本版不增加任何供應商呼叫，PDF／Web 模型與計次不變。

## 2026-09-04（v29.6.292 / Web 安全終點人性化）

- 安全降級不得把「逐句引用／驗證」等內部稽核術語丟給店員；改用自然同儕口吻交付最多三步可逆操作。清理連接詞後也要清理殘留標點，避免 `；，`。
- 來源、模型、額度與供應商呼叫數不變，完成快取升為 `EvidenceV9`。

## 2026-09-04（v29.6.291 / 網搜不可空轉）

- Google Search 已執行但 exact-model 引用不足時，不能把它寫成完全沒結果，也不能把模型草稿當已證實答案。只保留可逆、低風險操作；含推測、購買、其他型號、工程模式、韌體、產品能力、規格或數值的整句淘汰。
- 若沒有安全動作，回覆可現場驗證的方法、官網／三星客服或請 Sam 補 QA，並停止同來源循環。PDF rescue 與直接 Web 共用此契約；模型與呼叫次數不變，快取升為 `EvidenceV8`。

## 2026-09-04（v29.6.290 / Grounded Web 安全降級）

- Google Search 有 grounding 支持的操作方向，但不足以證明完整型號能力時，不再整段丟棄。只保留不含規格斷言、數值、購買、工程模式與推測的低風險動作，並標示非官方及型號適用性未完全確認。
- 未驗證全文仍不可轉交；精確型號功能仍須同一來源直接支持。PDF rescue 與直接 Web 共用此規則，快取 schema 升為 `EvidenceV7`。

## 2026-09-04（v29.6.289 / 每則回覆顯示模型與合計費用）

- 每則正式回覆最後顯示 `本次約 NT$…｜模型：…｜剩餘額度`；QA／RULE 零生成顯示 `未使用模型`，多階段則依實際 request audit 列出所有成功生成模型。
- 合計費用由 `usageMetadata` 的 input、candidate output 與 thinking tokens 計算，Router／Fast／PDF／Web 同輪累加；缺 metadata 顯示待確認，不沿用上一題。
- 2026-09-04 官方 Standard 價格：2.5 Flash-Lite US$0.10／0.40、2.5 Flash US$0.30／2.50、3.7 Flash 至 2026-12-31 為 US$0.75／3.75（每百萬 input／output，output 含 thinking）。固定匯率 32 僅供估算，所以對外使用「約」。
- G8／M8／M7 等已由 CLASS_RULES 解出的系列別稱不叫 3.7 Router：共同規格走 RULE 交集，操作或型號差異直接列實際候選；只有句意／追問／複合主張真正不明才啟動守門。
- QA／RULE 或部分證據無法完成時，不再以「要不要網搜」中斷。可用手冊先查手冊，否則自動進唯一 Web SourceOperation；Web 仍受每日 5 次、冪等與成本守門。沒有可核對網頁證據時不得硬編，但必須交付已知內容、標明未證實的安全排查方向及官網／請 Sam 補 QA，讓流程終止。
...
- 重設通用操作改以「重設／還原動作」與「全部／所有設定」兩個語意條件組合，不再依靠固定中文詞序。
- 型號、SHA、頁碼、摘錄、scope 與選配功能守門均不放寬；快取 schema 升為 `EvidenceV6`，清掉舊版未完成結果。
- 最終實問已由 `S24F332.pdf` 第 22 頁回答 `Support → Reset All`，LOG 為 `pdfCalls=1 / webCalls=0 / supported`。

## 2026-09-04（v29.6.286 / 新手冊實查與重設檢索契約）

- 正式 TestUI 已證明 S24F332 新手冊會被真正掛載；首次實問失敗是守門漏認「重設所有設定 / Reset All」同義，並且舊 Query Rewrite 誤把一般顯示器導向 Tizen 詞彙。
- 現在通用 Reset All 可在官方 support-page family pattern／SHA／頁碼／摘錄全數通過時採用；選配功能仍 fail-closed。重設搜尋詞依完整型號 RULE 分為 Tizen 與一般 OSD，不再套錯選單。
- 此改動只修檢索與證據契約，沒有額外 LLM 呼叫或模型升級。快取 schema 升為 `EvidenceV5`，避免重播舊版 S24 失敗終點。

## 2026-09-04（v29.6.285 / 手冊命名、新機自動納管與共用頁面適用性完成鏈）

- M7 改用官方支援頁 v2510220 手冊 `S32CM703.pdf`；G95SD 以官方 ZIP 內已核對 PDF 建立 `S49DG952.pdf` 單型號 alias。舊 `S32CM703,S49DG952.pdf` 只保留回復，不再進正式選檔。
- S24F332 已從台灣官方支援頁取得 2026-06-19 v1.0 英文 User Manual，31 頁標準 PDF，以 `S24F332.pdf` 進 registry。封面 `S24F33*` 只在 support SKU/download `ModelName` 精確一致且星號只對應一碼時才可綁定，並標記 `exactModelInDocument=false`。
- 每日發現繁中 UM 永遠優先；台灣支援頁沒有繁中時才退到官方英文 UM，TW area、`UNI_TW`、格式、首頁、SHA 與 provenance 一項都不能省。S27F612 下載檔是 `NASCA DRM`、不是 PDF，禁止進 RAG；S27H802EFA 則已納入 H 世代手冊覆蓋稽核。
- PDF evidence 必填 `pageHeading` 與 `applicabilityExcerpt`，再和摘錄共同核對型號／系列；Odyssey Ark 專屬頁不得套用到 M7。
- 基本人工輸入切換可把問題裡的 HDMI 1/2、DP、USB-C 綁到手冊的通用「選擇訊號源」步驟，但不能據此證明介面能力或規格；Auto Source、PIP／PBP、KVM 不放寬。
- 模型與呼叫數不變；快取 schema 更新為 `EvidenceV4`，避免重播不含新適用性欄位的舊證據。

## 2026-09-04（v29.6.284 / 進階來源快取契約換版）

- 進階來源快取升為 `EvidenceV3`。證據 validator 或完成狀態有語意變更時，必須同步換版，禁止沿用舊版失敗答覆而跳過真正 PDF／Web 查詢。
- 換版只淘汰不符合新契約的舊結果；同一新版內仍維持相同型號、主題與知識 fingerprint 的零成本重播。

## 2026-09-04（v29.6.283 / 手冊證據驗證與補救終點修復）

- v29.6.282 已能把 `S32CM703UC` 正確鎖到 `S32CM703.pdf`，但真人追問發現 feature validator 呼叫未宣告的 `escapeRegExp`，導致模型找到的第 14／28 頁證據在解析時被丟棄。
- 共用 helper 現已納入正式程式與契約測試；測試不得再以 stub 隱藏 production dependency。
- 證據驗證未通過時必須在同一輪真正執行一次受控 Web 補救，不能只向店員說「接著補查」後結束。

## 2026-09-04（v29.6.282 / 精確手冊鎖、系列 RULE 共識與來源完成度）

- v29.6.281 的正式 TestUI 證明：已鎖定 `S32CM703UC` 後追問 HDMI 2，舊選擇器仍展開系列兄弟並掛上 `S32DM702.pdf`，PDF＋Web 約 NT$0.2087 後仍答非所問。根因是選檔、來源身分與原題完成度契約，不是模型單純不夠聰明。
- 完整型號一旦由本輪文字或持久狀態確認，即成為 PDF strict lock；系列字只供理解，最後附件再以 exact-model allowlist 阻斷兄弟文件。實際沒有對應 URI／inline 的 PDF 不得由扁平索引冒充「可用」。
- Drive recovery、rolling refresh、完整同步及手動合併的 PDF metadata 統一保留／驗證 manifest SHA 與來源綁定；support-page-only 缺 SHA、SHA 不符及已知錯誤共用檔一律 fail-closed，不能在下一次排程又洗掉驗證資料。
- 手冊答案須同時通過「摘錄支持答案」及「答案完成原題」；功能、介面編號、操作動詞與明示數值任一漏答，只能是 partial／none。Web 補救把原題與擴寫搜尋詞分開，保留 `full / partial / none + groundingPresent`，並區分有網頁但不適用本型號與完全無 Grounding。
- RULE 的系列別對應只建立候選集合。詢問技術／規格時，僅在每個候選完整型號的精確 RULE 都有該欄且值一致時直接回答系列共通值；缺值或不同才問代別。操作、排障與可選功能仍需精準 QA 或同型號手冊，禁止以同系列推論。
- 模型與呼叫數不變：Router 3.7 Flash low（條件式）、Fast 2.5 Flash-Lite、PDF／Web 2.5 Flash；沒有藉本修復換模型或導入 managed File Search。

## 2026-09-04（v29.6.281 / 官方手冊來源鏈與最新檔治理）

> **現行優先契約**：本節覆蓋下方歷史版本中與 Router 模型、術語資料、手冊適用性、Web 證據、跨日追問及來源快取衝突的舊說明。

### 官方手冊資產治理（2026-09-04）

- G95SD／`S49DG952SC` 官網最新版仍為 2024-10-02 v2312130 共用 e-Manual ZIP；其繁中 PDF 與舊 `S32CM703,S49DG952.pdf` bytes 相同。舊合併檔保留作回復，但正式選擇排除並優先單型號文件；因內文未明列 G95SD，不得作型號專屬 capability／operation 證據。
- M7／`S32CM703UC` 的 2026-01-16 v2510220 新繁中 PDF 已加入 `config/manual_registry.json`。空白封面文件必須以官方支援頁 SKU、下載網址 `ModelName` 與 SHA／provenance 三向綁定，且標記 `exactModelInDocument=false`。
- 共用手冊內「依型號可能不支援」等句子，須另有 QA／RULE 或同段完整型號正向證據才能肯定。ZIP 必須先離線解包與驗證 PDF，不能直接進索引；registry/schema、唯一性、來源檔、PDF header、SHA 或綁定任一不符即 fail-closed，索引只以 staging 通過後的結果原子發布。
- 日常自動查新與 Files URI 更新不需要 `/重啟`；本輪不換 Router／Fast／PDF／Web 模型，也不增加模型或來源呼叫成本。
- 一對一 LINE 的文字、圖片與來源 postback，每個有效事件都必須在進入 QA／RULE／Router／PDF／Web 前啟動一次等待動畫；同一事件只呼叫一次。群組／多人聊天室受 LINE 官方限制無法顯示，TestUI 不呼叫 LINE API；不可用 Push 訊息替代。
- v29.6.281 起現行額度為一般 10 題／日、官方手冊 2 次／日、網路解答 5 次／日；本列覆蓋後方歷史章節中的 20／5／10。三者獨立，轉進階來源時一般題只退一次，系統 Web rescue 仍用獨立每日 3 次上限。

- G9 後方燈效事件不是 PDF 沒有執行：雲端 LOG 記錄到五次 PDF 呼叫。失敗根因是共用手冊的 Core Lighting／Eclipse 章節明寫「依型號而定，可能不支援」，舊驗證器卻把它當成 `S49DG952SC` 的正向證據，又讓 CoreSync、Core Lighting+、Infinity Core Lighting、Eclipse Lighting 等相似名稱互相替代。這是 Evidence 契約問題，不是單純模型智力不足。
- 本版只將條件式 `RouteAnalysisV1` 規劃器改用 `models/gemini-3.7-flash`、`thinkingLevel: low`。它只在模糊型號／系列、複合題、省略式追問、部分覆蓋或規則衝突時呼叫一次，採 Structured Output，沒有工具、沒有 `answer` 欄位，不能產生產品事實。精準 QA、完整 RULE、人工核對片段、指令與明確來源按鍵都先由程式完成，`routerCalls=0`。
- Fast 仍為 `models/gemini-2.5-flash-lite`，PDF／Web 仍為 `models/gemini-2.5-flash`；來源順序仍是 `QA／RULE／verified Evidence → PDF → Web`。精準 QA／RULE 已完整回答就停止，不可為了「更聰明」再多呼叫模型。
- 三星術語應資料化拆列：`definition` 只說明名詞；系列／型號 `capability` 必須精確標示適用 scope；操作 `operation` 另需同一 canonical 型號的手冊路徑證據。CoreSync、Core Lighting+、Infinity Core Lighting、Eclipse Lighting／Eclipse Sync 不得互換；definition 也不能證明某型號支援。
- 2026-09-04 官網盤點已落成 154 筆 `術語_` 與 9 筆 `能力_完整型號` 資料列；術語總數包含舊雲端 key `術語_OdysseyHub`、`術語_MiniLED`、`術語_AIUpscaling` 的安全 legacy alias，三者只導向 canonical，不能證明型號支援。術語列只做名稱／別稱理解，包含首頁的 FHD、更新率、反應時間、曲面、內建喇叭、Smart TV 與 USB-C 視訊等上位詞；能力列才可證明所列完整型號。`Adaptive Sound` 的一般、+、Pro 版本，以及 `Auto Source Switch` 與 `Auto Source Switch+` 均分開，禁止省略後綴後互相套用。每筆能力快照須保留 `checkedAt` 與 Samsung 台灣官方 URL；系列詞只能產生候選，不能宣稱整系列支援。
- 共用手冊只要有「依型號而定／可能不支援／部分型號」且沒有同段正向綁定完整型號，就不可當正向 capability 或操作證據。Web 也必須由同一來源同時支持 canonical 型號與本題主張，不得跨網站拼接或輸出無引用的模型草稿。
- 已確認型號與 canonical 主題跨日保留；新完整型號或管理員 `/重啟` 才清除。這讓「那在哪裡開？」可承接數天前的同款話題，又不會把舊產品證據借給新機。
- 同一台北日期只在 `來源＋完整型號＋正規化主題＋Evidence schema＋來源指紋` 一致時重用結果。PDF 指紋取自 Drive fileId、updatedAt、size／identity；Drive PDF 變更必刷新 Gemini Files URI，禁止續用舊文件與舊答案。
- 本版不宣稱消滅所有幻覺；完成標準是結構性阻斷已知的錯型號、錯術語、條件式手冊誤採、跨站拼證據、舊 URI／舊快取及追問斷裂。所有來源仍無證據時，保留未知並安全收尾，不補單題 Prompt 或看似合理的答案。

## 2026-08-28（v29.6.279 / 競品範圍守門前移）

- 競品螢幕比較現在會在一般螢幕情境放行前由程式攔截；跨裝置連接三星螢幕仍正常處理。這能避免多花一次 Fast 費用後只得到「再查網路」。
- 回覆改為店員同儕式安全收斂，零 Router／Fast／PDF／Web 呼叫；不得用品牌單題 Prompt 或更貴模型補救分類順序錯誤。

## 2026-08-28（v29.6.278 / 店員同儕角色一致化）

- Fast 與 Router 的唯一角色統一為三星螢幕門市店員內部同儕助手；移除殘留的「台灣三星官方客服」與制式競品拒答範本。
- 缺本機證據的時效題只交由程式接續既有 Web 成本／Evidence 守門，不再先問店員是否搜尋；模型與來源呼叫數均未增加。

## 2026-08-28（v29.6.277 / 條件式 RouteAnalysisV1 主張規劃器）

- 固定順序為 `程式控制／產品身分／免費證據 → QA／RULE → PDF → Web`。指令、postback、精準 QA、完整 RULE、人工驗證片段與明確來源按鍵不呼叫 Router，`routerCalls=0`；只有模糊系列、可能追問、複合主張、部分覆蓋或路由衝突才呼叫一次。
- Router 使用 Gemini 2.5 Flash-Lite、thinking=0、Structured Output、短 Prompt、無搜尋／檔案工具、無答案欄位與零重試。只能從程式候選 index 選型並拆 claims；低信心、429、逾時、格式或語意驗證錯誤直接 fallback，不得創造事實、扣額度、授權來源或指定 PDF。
- `SEMANTIC_ROUTER_MODE=off|shadow|conditional`，v29.6.277 正式預設為 `conditional`；明確 `off` 才停用，Shadow 只記稽核、不改答案與額度。TestUI `?router=` 僅作用於該請求。Conditional 接管後，舊 AUTO_SEARCH／NEED_DOC 暗號只留 fallback／歷史稽核，不可再形成第二套路由。
- 每個 claim 必須以同筆 evidence 關聯來源、型號範圍、頁碼／網址、摘錄與限定條件。PBP 兩側 120Hz 只有同一證據明文綁定才可回答，禁止以分散關鍵字或整機最高值推論；partial 保留有效內容，只升級未解 claim。
- 現行 PDF 仍為 Gemini Files API 選定單一正確文件後以 `file_data` 送入 Gemini 2.5 Flash，不是 File Search。本版不換模型、不新增搜尋工具；File Search 必須另案用固定手冊題做相容性、引用率、延遲與有效答案成本 A/B，不與 Router 同版遷移。
- 稽核欄位至少包含 `routerCalls、routerCacheHits、plannerLatencyMs、routerCostTwd、routePlanValid、claimRoutes、selectedModel、pdfCalls、webCalls、finalCoverage`。接管門檻為 20 題至少 19 題到達正確終點、安全題 100%、JSON 有效率 99%、單次 Router 不超過 NT$0.01、P95 額外延遲不超過 2 秒，並維持等價題 PDF／Web 次數不增加。
- Apps Script 版本逼近 200 上限；先用 HEAD 與 request-scoped TestUI 驗證，正式發布前盤點容量、保留正式與回復版本，不得為 Shadow 反覆建立版本。

## 2026-08-27（v29.6.276 / PDF 證據數字與 Web 完整句守門）

- 正式 TestUI 證明正確 PBP 手冊與頁面已被選中，失敗點是 Evidence 驗證器把回答中的完整型號數字誤當規格數字。現在先移除已由型號適用性另行驗證的型號 token，再核對真正規格數字；未被摘錄支持的 120Hz 等內容仍 fail closed。
- PDF 位置題先查目錄與命中章節，答案必須包含 `功能分類 → 設定項目`；Web 安全補救以完整句過濾，不再把逗號後文字拆成殘句。這是共用契約修復，沒有新增 PBP 單題 hard-code。
- Fast／PDF／Web 模型、thinking 與費率均未改動，沒有新增 File Search、Gemini 3.x 或額外生成階段。

## 2026-08-27（v29.6.275 / 固定 20 題全局 RAG 與追問契約）

- 正式 TestUI 固定 20 題基準涵蓋規格、系列、操作、故障、比較、未知型號與自然追問；修復只改共同契約，不加入題目字串特判。
- 回答鏈固定為產品身分 → QA → RULE → 已核對手冊片段 → 整本 PDF → Web。型號限定／模式限定條件不得被一般規格值覆蓋，Fast 無證據但已有唯一型號與手冊時直接完成 PDF 查證。
- 手冊輸出以 `coverage=full/partial/none` 驗證每個主張；入口題必須有路徑。部分答案保留手冊證據，再自動補一次 Web，不把已確認內容丟掉。
- 正式仍是 Gemini 2.5 Flash-Lite（Fast）與 2.5 Flash（PDF／Web），thinking=0；目前使用 Files API 精確掛載整份 PDF，不是 Google File Search store，沒有在本版換模型或增加問答呼叫。
- Files URI 改為每 4 小時輪替續期 10 本、每日增量同步；真人查詢保留指定單檔自癒。約百本可在 40 小時完成一輪，避免全量重傳撞上 GAS 上限。

## 2026-08-27（v29.6.274 / 結構化 QA 可信命中先於別稱選型）

- 完整型號或系列別稱＋至少兩個結構化意圖詞，且最高候選分數至少 55、領先第二名至少 8 分時，可直接採用既有 QA；不再只靠固定 68 分門檻把正確 QA 擋掉。
- 任一證據條件不足仍拒絕命中並進正常選型／PDF 流程。這是通用 scorer 契約，不得改成 Netflix、M8 或其他單題 hard-code。
- 沒有新增 Prompt、模型呼叫或費用；產品身分、精準 QA、RULE、已核對片段、整本 PDF、Web 的既定順序不變。
- 正式 @1456 已以 M8 QA、M7 跨輪沿用、S27DG5 不完整型號實讀 PDF、G8 多候選後 PDF→Web 補救驗收；新增實際費用 NT$0.3386。

## 2026-08-27（v29.6.273 / 產品身分與來源路由順序）

- 先解析本輪完整型號／前段／系列別稱／具名家族，再考慮跨日型號。句首 Smart 未給 M5/M7/M8/M9 時只補問一次代號；不相容舊型號僅本輪停用，不刪除。
- QA、精確 RULE 與已核對手冊片段全部先於整本 PDF。已移除 QA 前 Operation Source Gate；自動手冊不得用假的 `priorFastChecked=true` 跳過免費資料。
- 候選保存完整 CLASS_RULES 集合（最多 50），LINE 每頁 8 款；手冊選型另要求正式 PDF 索引覆蓋。模型、Prompt 內容與價格不變。

## 2026-08-23（v29.6.272 / 比較題依已列證據給選擇摘要）

- 使用者問怎麼選時，規格逐欄表後要有一句可行動摘要；只允許引用同答已列 RULE 差異。
- 可比較明列／未列支架與數字型對比；不可自行宣稱 IPS／VA 哪個一定較好，也不可補未要求欄位。
- 摘要由 deterministic formatter 產生，模型與費用皆不變。
- 正式 v29.6.272 已以修圖比較原句真人驗證為 deterministic、NT$0；同輪再驗證 `S32D806` 自然 KVM 追問，僅掛 1 份正確 PDF，取得第 17、19、21、33 頁與 `USB Source Setup／切換 USB 訊號源`，`pdfCalls=1 / webCalls=0 / thinking=0`。整輪測試約 NT$1.9696，模型、Prompt 與付費呼叫策略皆未提高。

## 2026-08-23（v29.6.271 / 年份描述必須參與候選縮選）

- 世代碼 T/A/B/C/D/F/H 對應 2020–2026（E/G 跳號）；RULE 有明文年份時以明文為準。
- 年份、尺寸、解析度、更新率、面板、平／曲面與供電瓦數全部採 AND；不符合年份的舊款不得留在選單。
- 多個同年外觀版本仍符合時保留選單，禁止猜色系。這是零模型資料規則。

## 2026-08-23（v29.6.270 / TestUI 載入不可偷偷重置聊天室）

- 同一分頁重新整理仍是同一 `TEST_USER_ID`，必須保留型號、比較、配額與來源狀態，才能模擬 LINE 使用者隔天接續提問。
- 只有明確按「重置」或管理員輸入 `/重啟` 才可呼叫 `clearTestSession()`；`window.onload` 嚴禁執行清除。
- 這是測試環境擬真修復，不改正式 LINE 回覆、不增加任何模型費用。

## 2026-08-23（v29.6.269 / 自然追問只保留必要前題脈絡）

- PDF 為省 token 維持單一訊息，但含代稱或省略訊號時可附一行上一個使用者問題；不得帶整段歷史或前答，獨立新題也不得污染。
- 型號核心可和同核心的完整變體／地區尾碼證據對齊；只有裸核心 target 可用此前綴規則，完整型號彼此仍需精確核對。
- 操作題中的介面名稱不是再次詢問規格；沒有明問數量、瓦數、版本或支援時，不合併 RULE 規格長文。
- 這三項是對話、身分與輸出契約，不是 KVM 或 S32D806 特例，也不增加 Gemini 呼叫。

## 2026-08-23（v29.6.268 / 單純問入口時只顯示直接答案）

- 手冊即使找到更多已驗證內容，也不能把使用者沒問的圖示樣式、限制與 OCR 雜訊全部堆上 LINE。
- 問「在哪個選單／設定」且不是故障排除時，只從已通過 Evidence Guard 的主張中保留入口與必要進入步驟，最多 2 筆；故障題仍保留完整必要步驟。
- 這是零模型的終答相關性篩選，不新增 Prompt 特例、不重查 PDF、不增加費用。

## 2026-08-23（v29.6.267 / 手冊答案採逐主張 Evidence 契約）

- PDF 每筆 evidence 必須一起提供可直接顯示的 `supportedAnswer`、頁碼、適用範圍與同頁摘錄；頂層 answer／operationPath 一律忽略，避免未綁證據文字混入終答。
- 驗證以主張為單位：2/3 有效就保留 2 筆，只有 0 筆有效才視為失敗。答案按主張去重、來源頁碼另外去重，因此同一頁的不同有效資訊不會互相吃掉。
- 錯型號、摘錄外數字、沒有直接否定證據的否定句、網址、內部 marker 及 `found=false` 卻帶 evidence 都 fail closed；任何 `[MANUAL_*]` 不得出現在 LINE。
- 不新增題型特例、不增加 Gemini／Web 呼叫、不更換模型；維持 Fast 2.5 Flash-Lite、PDF／Web 2.5 Flash 且 thinking=0。

## 2026-08-23（v29.6.266 / 追問型號必須一路傳到付費來源）

- 自然代稱或持久產品狀態解析出的型號，要同步到路由原題、最近問題與來源 pending；舊 `suggested_models` 不得在終答後反向覆寫。按手冊後查到另一款 PDF 屬 hard fail。
- 通用操作語序包含「哪個選單打開／什麼設定開啟／功能如何切換」。完整型號且 QA／RULE 無操作證據時直接查 PDF，不先生成一輪無證據答覆再叫使用者按手冊。
- 來源結果只在同 `GAS_VERSION` 內做 10 分鐘去重；新部署不得沿用上一 patch 的結果，以免舊錯答遮蔽修復。模型與成本配置不變。

## 2026-08-23（v29.6.265 / PDF 依型號證據與 schema 對齊）

- 正式題已掛正確 `S32FG502EC` PDF 並找到第 49 頁，但 schema 允許 `依型號而異`、validator 卻拒絕，導致官方答案被丟棄並多做 Web。
- 現在三種 scope 與 schema 一致；`依型號而異` 必須在摘錄明載目前 target model 才能通過，錯型號與無型號摘錄仍 fail closed。混合共通＋專屬 evidence 一律標「型號明確」。
- 不保留無法對應主張的部分 answer、不放寬跨型號防線、不新增供應商呼叫或換模型；目標是讓已取得的合法 PDF 證據完成回答。

## 2026-08-23（v29.6.264 / 選型、比較與自然追問共用契約）

- 十題正式 TestUI 旅程確認，失敗共因不是模型等級，而是候選先截前 10 款、舊 pending 未隨新題結束、雙型號比較只回固定少數欄位。禁止再用 Prompt 或單題特例補洞。
- 系列別稱先掃完整 CLASS_RULES（內部最多 50 款），用首句的尺寸、解析度、更新率、面板、平／曲面與供電做 AND 縮選，再套畫面上限。手冊模式最後強制限定實際 PDF 索引；無覆蓋時回空，不得把 RULE 型號冒充有手冊。
- 候選、原題與模式同為 600 秒。純描述只在當前候選內縮選；帶功能問句或不同完整型號時結束舊候選，保留跨日單一產品記憶。`#型號:` 只接受本輪候選，舊泡泡不得接回歷史或破壞目前題目的退款 hold。
- 雙型號比較依原題指定欄位逐項輸出兩款的 RULE 明文或「官方規格未列此項」。比較脈絡保留 30 分鐘；前者／後者與唯一尺寸描述可連續追問，含糊的「這台／它」才列原兩款供選擇，選定後接回原題且不重複計次。
- PDF 主要型號以使用者實際選定值優先；共享手冊的推定型號不得覆蓋。Fast 仍是 Gemini 2.5 Flash-Lite，PDF／Web 仍是 Gemini 2.5 Flash 且 thinking=0，本版沒有增加供應商呼叫或換模型。
- 必守回歸：G5 27 吋 QHD 180Hz IPS 要留下 DG／FG 兩款；G5 候選後問 S32D806 不得沿用舊選單；S27H704EAC／S32D707EAC 比較需完整回答面板、對比、支架，後續指代不得落回比較前舊型號。

## 2026-08-23（v29.6.263 / 多型號共用 PDF 不得跨型號引用）

- 手冊檔名與掛載型號正確，仍不代表同頁每個圖表都適用；證據摘錄必須可直接核對當前鎖定型號。
- 摘錄只列其他型號、或標「型號明確」卻沒有型號限定，一律拒絕；封面型號清單不能當功能證據。
- 已以官方手冊第 12 頁建立資料層證據：HG806 無 USB-C 影像／PD；HG802 有 USB-C 影像與最高 98W PD。以後補資料與測試，不新增單題路由或 Prompt 特例。
- 維持 Gemini 2.5 Flash-Lite／2.5 Flash 原分工、原呼叫次數與費用守門。
- 正式 TestUI 已驗證 G8 模糊選型、「32 吋 6K」描述選型與原題延續；終答以官方手冊第 12 頁已驗證片段零供應商呼叫完成。

## 2026-08-23（v29.6.262 / Web 系列名稱不綁死語序）

- `M8 智慧螢幕` 與 `Smart Monitor M8` 都需以同一款完整型號的精確 RULE 核對。
- 系列詞與別稱在同句近距離內可前後互換；錯家族與相鄰系列仍拒絕。
- 不增加供應商呼叫與費用。

## 2026-08-23（v29.6.261 / 非官方 Web 證據不再硬卡台灣完整 SKU）

- Web 文章只寫 Smart Monitor M8 等正式系列名時，先用目前完整型號自己的 CLASS_RULES 精確列核對身分；不能靠 Prompt 或相近型號猜測。
- 完整型號、去地區碼型號或正確家族＋系列才可通過；錯家族、相鄰系列與明說其他型號一律拒絕。
- 僅低風險故障排查允許以完整回應中的正確系列身分銜接 grounding 支持步驟；拆機、刷機與非官方韌體不得放行。
- 不增加模型／搜尋呼叫，既有主題詞、引用、完整句與危險內容守門不變。

## 2026-08-23（v29.6.260 / 完整型號操作題零 Fast 直進手冊）

- 「在哪／哪裡開／去哪／位置」是通用選單操作意圖，不得因同句有 HDMI 就只回埠數。
- 精準 QA／RULE 未完成且型號已知的操作題，零 Fast 直接查免費片段或完整 PDF；一般題額度退回。
- 純規格 RULE 必須在操作自動手冊守門之前有優先回答機會，避免回歸。

## 2026-08-23（v29.6.259 / 介面時序題不得被泛用片段截斷）

- 「Dual Mode」等已驗證操作片段不是特定介面訊號時序的證據。
- 介面×解析度／更新率題必須跳過無關片段，讀完整 PDF 的輸入訊號表後才能下結論。

## 2026-08-23（v29.6.258 / 介面訊號時序必須查手冊）

- 介面版本、解析度與更新率是獨立 RULE 欄位，不能相互推論同時可達。
- 指定介面的解析度／更新率相容題，精準 QA 未命中後要零 Fast 直接查手冊訊號時序表，並退回一般題額度。
- 純問「有幾個 HDMI」仍可由 RULE 零成本直答，不得因此回歸影響。

## 2026-08-23（v29.6.257 / 一般與手冊候選共用描述選型）

- 一般問答與手冊候選不得再有兩套「自然描述→型號」邏輯；一律使用同一解析器與 CLASS_RULES 證據。
- 唯一命中才自動選型並接回原題；歧義或無命中要保留原題與候選，零 LLM、零額外計次。
- 一般候選先走 QA／RULE，不強制每款都有 PDF；真正進手冊路由時，候選仍只能來自實際 PDF 索引。

## 2026-08-23（v29.6.256 / 自然描述選型不再遺失原題）

- 正式 TestUI 實測：G8 已列出 9 個有 PDF 的候選後，輸入「我看的是 32 吋 6K 那台」仍被當成新問題，證明自然選型入口沒有接上 pending 狀態。
- 新增零模型描述 resolver：僅在既有候選內，以 CLASS_RULES 的尺寸、解析度、面板、平／曲面、更新率、雙模等明確條件做 AND 比對；唯一命中才選型，不做模糊分數或最高分猜測。
- 選中後使用「完整型號＋原 draftQuery」續跑；多重／零命中都保留原題與候選，不讀 PDF、不扣題次或手冊額度。候選 RULE 一次批量讀取，避免冷快取逐款掃 Sheet。

## 2026-08-23（v29.6.255 / 費用顯示納入 Gemini 思考 token）

- Gemini 回傳的 `candidatesTokenCount` 只代表可見回答；成本公式還要加上 `thoughtsTokenCount`。兩者合計為 billed output，再套用該模型的 output 單價。
- 10 個既有 Gemini 計價位置已改用同一 helper；稽核紀錄同時保留 candidate、thought、billed output，方便之後核對正式帳單。
- 本版只修估價與稽核，不更換模型、不改 Prompt 本文、不改生成參數，也不增加任何 API 呼叫。

## 2026-08-23（v29.6.254 / 自然追問不能因省略型號而失去證據）

- 真人式 TestUI 追問證明：對話已鎖定 `S49DG932SC`，Fast 也答對一般 HDMI 不能直接插 Micro HDMI，舊 AnswerEnvelope 卻只從本輪原句抽型號，因此刪除正答並再次要求查來源。
- 精確 RULE 路由改為沿用已確認型號；能由同一型號規格回答時，直接零模型完成，不要求使用者重複型號。
- 接頭與配件以欄位資料判斷：標準 HDMI／Micro HDMI 分開辨識，只有實際配件欄明載轉接線才能說隨附。這是通用證據契約，不是 G932 單題字串修補。
- 不更動模型、Prompt 本文或價格守門。

## 2026-08-23（v29.6.253 / LINE 回覆最後出口防空陣列）

- `G932如何開啟PBP` 實際已查完正確手冊並生成答案，但空的 Quick Reply 陣列使 LINE API 回 400。這是回覆封裝錯誤，不是 PDF 或模型失敗。
- 沒有按鈕時完全不輸出 `quickReply`；LINE API 前再做一次非空陣列檢查。新增合約測試防止正確答案再次在最後一步消失。
- 同題已完成結果在原 10 分鐘內可沿用前一 patch，修復部署後不重查 PDF、不重扣次。

## 2026-08-23（v29.6.252 / 移除沒有讀取者的 PDF 快取）

- 正式 LOG 把混合手冊題拆開後，Gemini 實際約 5.03 秒，但 PDF 選檔後到生成前仍有額外等待。稽核確認 `${userId}:last_kb_files` 只寫不讀，對選檔、回答與防重複扣款都沒有作用。
- 已移除該同步 ScriptCache 寫入與整份 PDF 清單序列化；本題 PDF 仍由 `getRelevantKBFiles()` 回傳值直接掛載。
- 不更換模型、不改 Prompt、不加模型呼叫；QA／RULE → PDF → 必要時 Web、20／5／10 額度及 NT$0.35 PDF 上限均不變。
- 靜態測試禁止死快取名稱再出現；正式驗收需確認 `pdfCalls=1 / webCalls=0`、正確手冊與頁碼。
- 正式單題驗收：總耗時 17.35 秒，PDF 選檔到生成設定 0.87 秒；Gemini 2.5 Flash 4.43 秒、NT$0.1313、1 次 PDF／0 次 Web。答案正確合併規格與第 23、24 頁手冊內容，沒有重複 HDMI 結論。

## 2026-08-23（v29.6.251 / 手冊只做一次免費預檢）

- 正式雲端 LOG 證實，同一個完整型號手冊題在真正掛 PDF 前，以完全相同參數查了兩次 QA／RULE。第二次沒有新資料，只會增加等待與紀錄量；現已移除。
- 保留選型後、扣次與建立 operation 前的第一次免費預檢：QA／RULE 命中仍直接回答且不扣手冊次數；未命中才真正查 PDF。額度、防連點、快取、PDF 成本上限與失敗後 Web 補救均不變。
- 正式 Prompt 同步一律轉 PowerShell 7、以 UTF-8 bytes 傳送並回讀版號／字數。中文被截短時必須中止發布，不得讓不完整 Prompt 上線。
- 本版不換模型、不加第二次生成：Fast／Polish 維持 Gemini 2.5 Flash-Lite，PDF／Web 維持 Gemini 2.5 Flash。

## 2026-08-22（v29.6.250 / 單題快路徑、證據去重與 PDF 續期韌性）

- 正式 v29.6.249 實測證明精準 RULE 本體約 1.5 秒完成，但 TestUI 單題發送前額外的 checkPdfCost RPC 使體感多等約 12 秒。直接單題現在略過該前端 RPC，批次仍保留；後端 countTokens、NT$0.35 上限、每日額度與冪等完全不變。
- RULE／PDF 重複改以「同型號＋同介面＋同核心值」決定式去重，不增 LLM 與模糊相似度。同值只留一次，操作步驟與頁碼保留；數值衝突採 RULE 並寫稽核 LOG，不把矛盾雙答案顯示給客戶。最後只有一個自然資料來源頁尾。
- PDF 過期先以 driveFileId 直取本題手冊，只有舊索引缺 ID 才掃描，找齊後立即停止。每日同步若部分失敗，只沿用失敗檔的舊 URI，保留其他成功新 URI；維護欄位改用正式 expirationTime。
- 費用契約不變：Fast／Polish 為 Gemini 2.5 Flash-Lite，PDF／Web 為 Gemini 2.5 Flash，LLM_PROVIDER=Gemini；不新增模型、供應商、潤飾或第二次生成。
...
- Rich Menu 是 LINE 平台上的獨立資產。`release_existing_webhook.ps1` 更新 GAS Webhook不代表 default Rich Menu 已建立或仍有綁定；每次選單異常須依序 inspect 目前 default → validate／publish → 回讀 default ID。
...
- 同一題詢問多個規格面向時，精準 QA 必須完整回答；沒有精準 QA 時不得被單一 RULE 欄位提前截斷。產品答案放 QA2，不把型號特例寫回路由。
- 規格數值比對必須有單位邊界；例如 `263.5mm` 不得命中 `3.5mm` 音訊孔。
- 免費資料不足時只顯示手冊與網路來源；未按手冊不得自動送 PDF。按網路則直接執行，不再多問一次。
- 明確功能題若該型號規格沒有證據，必須在 LLM 前零成本停下，並同時給手冊與網路選項。

- 新增 `qa_knowledge.gs` 作唯一 QA knowledge layer；新資料以 `QA2:` JSON 單列存入 `QA!A:A`，舊 `[標籤] 問題 / A：答案` 保持讀取相容。
- `answer` 只保存結論、事實、步驟、提醒與替代方案；LINE 白話與排版由共用 renderer 產生。產品特例只能補 QA2 資料與測試，不得再補 `handleMessage` 題型分支或 Prompt 句子。
- 同步建立固定 record shards 與 16 桶倒排索引；每題先用型號硬隔離、別稱／系列與詞彙排名，Fast 最多只注入 6 筆 QA，不再整包載入。
- 12 筆已核對手冊片段已從 `linebot.gs` 搬到 `QA.csv` 的 `evidence.type=manual_chunk`；型號、題意與排除詞都符合才可零 PDF 回覆。
- `/紀錄`、AutoQA、`writeQA` 新資料統一轉成 QA2；`tools/sync_qa2_evidence.ps1` 以 idempotent upsert 同步正式 QA Sheet。
- `[AUTO_SEARCH_PDF]` 僅能推薦來源，型號已明確也不得自動執行；使用者按「查官方手冊」才授權。
- Google 官方仍建議重複使用大型 PDF 採 Files API；新版 File Search 可語意切塊並回傳頁碼 citation，但僅支援 Gemini 3.x 且不能同次搭配 Google Search。先以代表題 shadow A/B 比較答案率、引用正確率、延遲與有效答案成本，本版不直接替換正式 2.5 路徑。
...
- 一般 10 題、手冊 2 次、網路 5 次是三個獨立額度。PDF 已付費但無證據時可做一次系統 Web 補救；不扣使用者網搜額度，每聊天室每日最多 3 次，且禁止再次重試或跨回 PDF。
...
2. **再官方手冊**：QA／RULE／已核對片段不足且仍有手冊型未解 claim 時，由程式自動建立一次 manual SourceOperation，不先回一個要求重按的空答案；「查官方手冊」按鍵則是使用者主動指定同一路徑，直接執行、不二次確認。缺完整型號時先列實際 PDF 索引候選，選完直接查；PDF 生成階段只讀手冊。手冊／網路各用自己的額度，不重複扣一般 10 題；已鎖定型號與主題跨日沿用，直到新完整型號或管理員 `/重啟`。
3. **再網路搜尋**：價格／通路／活動／據點／最新資訊依 plan 進 Web；PDF 無原子證據時可自動補救一次，不再要求店員反覆按鍵。只有同一來源同時支持 canonical 型號與主張才標 `[來源:網路搜尋]`，無引用草稿不得交付。
...
10. **模型成本防呆**：Fast／Polish 固定 `models/gemini-2.5-flash-lite`，PDF／Web 固定 `models/gemini-2.5-flash`；只有條件式 Router 使用 `models/gemini-3.7-flash`、`thinkingLevel: low`、無工具與無答案欄位。不得使用 `latest`／`exp`，不得把 3.7 擴散到每題回答；模型或費率變更須先做成本審查並更新靜態守門。
