v29.6.324：費用與證據契約修正，當次狀態見 AI_CONTEXT.md；歷史以 Git 保存。
...
- 原 6 份／17 型號候選擴為 81 份型號範圍登錄／136 型號；相同 PDF 內容去重，並非 81 本不同手冊。官方不可解析、型號矛盾與缺完整使用手冊仍列明，不以產品指南冒充。
- 發現 Samsung UM 分類亦包含較新的 Product Guide：正式每日選檔改按文件角色排除產品指南／快速入門，避免取代操作手冊。
- 頁索引先由編輯者上傳、SHA 讀回後原子切 MANUAL_ACTIVE 並保留 PREVIOUS；上傳分批可續傳，正式新增索引外置 Drive，不把全庫文字塞進每題。
- 模型、Prompt、來源配額與 Rich Menu 不變；真實提問曝露雙欄交錯與模型遗漏選單階層，改用原表格路徑／跨頁證據、SCFU與唯一代號統一解析，不增加產品特例。EvidenceV21失效舊答快取。
- 最終Chrome三題：G932 PBP（35頁）、F24護眼（18／20頁）、同聊天室改問H704自我診斷（41頁）均正確路徑，每题PDF1／Web0／Router0；約0.0098／0.0067／0.0023元。失敗迭代保留於本批實測報告，不改寫通過。
- 雲端active81／models136／uniqueIndexes47／missing空；驗證帳NT$2.06219776，本批新增0.47268480。正式guarded release @1486，local／HEAD／formal health版本build一致，30個版本；static／contract／57整合／全庫／diff通過。手機LINE及全案20旅程未混稱本批通過。
...
- 改成自然店員同儕口吻，並補上連接詞移除後的前導標點清理。只改呈現與同題完成快取，不改證據範圍、模型或呼叫數；schema 升為 `EvidenceV9`。
...
- 移除兩類沒有必要的 3.7 Router 啟動：已知完整型號且明確要查手冊、明確價格／庫存／活動等 Web 時效題。這些由程式直接依 QA／RULE→PDF→Web 政策處理。
- canonical topic 已有 Router claims 時，省略式連續追問直接沿用，避免每輪付費；新的模糊產品或複合主張仍可再規劃一次。
...
- 全域修正數字證據比對：先移除已由型號適用性守門另驗的型號 token，再比對真正規格數值；同時以正例「型號前綴＋正確選單路徑」與反例「摘錄沒有 120Hz 卻亂報 120Hz」建立契約。
- Web fallback 改以完整句保留安全動作，不再用逗號將一句話切成 `然後將其開啟`／`有些使用者提到` 等殘片。模型、thinking 與費率未變。
...
- deterministic 比較器現在以已列差異補摘要：H704 有明列人體工學支架；D707 官方對比數值 3000:1 高於 1000:1。對 IPS／VA 不自行判優劣，維持使用者要求的「只照官方規格」。
- v29.6.272 發布後，第 10 題正式 TestUI 重問通過：逐欄規格後正確補出支架與官方對比數值的選擇摘要，`paidCalls=0 / pdfCalls=0 / webCalls=0`。
- 最後再以 `S32D806 規格題 → 桌機 DP／筆電 Type-C 共用鍵鼠` 自然追問驗證手冊鏈：只掛 `S32D806.pdf`，`pdfCalls=1 / webCalls=0 / thinking=0`，回覆涵蓋 USB Hub、`USB Source Setup` 與第 17 頁「切換 USB 訊號源」，證據頁為 17、19、21、33；本題 NT$0.1971。
- 本輪十題與必要修復重驗合計約 NT$1.9696，未超過 NT$2。正式 Webhook 已更新至 Apps Script `@1454`；health、HEAD 與正式 TestUI 均為 v29.6.272。模型與 Prompt!C3 未變更，也未執行付費 Web 搜尋。

## 2026-08-23（v29.6.271 / 第 7 題 M7 年份描述未參與縮選）

- `M7＋32吋＋2025＋4K＋USB-C 65W` 零成本進候選，但畫面列出 FM 2025 兩款與 BM 2022 兩款，證明 descriptor 只認尺寸／規格，完全忽略使用者已說的年份。
- 新增通用 Samsung 顯示器世代年份解析，RULE 明載年份優先；年份納入 AND 過濾。契約確認本題只剩 `S32FM702UC／S32FM703UC`，仍不猜兩款外觀版本。
- 本輪沒有 Gemini 呼叫，累計成本維持 NT$1.7725。

## 2026-08-23（v29.6.270 / 第 6 題揭露 TestUI 每次載入都刪除持久狀態）

- 部署後重新整理 TestUI 再問 `這台底座能轉直嗎？升降多少？`，沒有沿用上一題 `S32D806`，反而呼叫 Flash-Lite 後回資料不足，成本 NT$0.0058；測試累計 NT$1.7725。
- 後端 `rememberSourceProductModel_()` 本來已同時寫 Cache 與 ScriptProperties。真正清掉它的是 TestUI `window.onload` 無條件呼叫 `clearTestSession()`；該函式還會刪比較、來源與配額，所以重新整理後看到 20/20 也是不實驗收。
- 移除載入時清除；保留明確「重置」按鈕與 runner 隔離呼叫。新增靜態契約，禁止 `window.onload` 再出現 clear，且確認明確重置仍可用。

## 2026-08-23（v29.6.269 / S32D806 KVM 追問暴露前題裁減與型號尾碼雙重缺口）

- 正式第 5 題 `桌機用 DP、筆電用 Type-C，同一組鍵盤滑鼠要怎麼設定才會跟著切？` 確實掛載 `S32D806.pdf`，`pdfCalls=1 / webCalls=0 / thinking=0`，成本 NT$0.1864，累計 NT$1.7667；但只保留第 21、16 頁，未回答第 17 頁的 `Switch USB`。
- 本機文字核對確認：第 17 頁對 S27D806UAC／S32D806UAC 明載「向右：切換 USB 訊號源」；第 19、21 頁分別說明 USB 上行／Type-C 啟用 Hub 與 USB 裝置連接。舊回覆只說 Source，沒有完成 KVM 問題。
- 根因一是 PDF 單訊息裁減把前題的 KVM 主題刪掉；根因二是 target `S32D806` 與手冊 `S32D806UAC` 未被證據守門視為同一核心，5 筆 evidence 只留 2 筆。另有已知 RULE 長文被不必要地重貼。
- 修正為條件式一行前題脈絡、裸核心與完整尾碼安全對齊、操作句介面詞不觸發規格長文；新增三項零模型契約，不加 KVM 特例、不增加供應商呼叫。

## 2026-08-23（v29.6.268 / 正確手冊回答仍需移除未問 OCR 雜訊）

- v29.6.267 真人第 3 題正式通過：`後者` 正確解析為 `S32FG502EC`，只掛正確 1 本 PDF，`pdfCalls=1 / webCalls=0 / thinking=0`，第 34 頁回答 `Game → Virtual Aim Point`，成本 NT$0.2160；測試累計 NT$1.5803。
- 同答另列手冊瞄準點圖示，但 PDF 文字層把圖示辨成 `5:3、53、23、613`。這不影響核心答案，仍會讓使用者覺得囉嗦且不自然。
- 新增通用終答相關性篩選：單純問選單／設定入口時，只保留逐筆驗證後的入口或必要步驟，最多 2 筆；故障排除維持完整。新增契約確認入口保留而 OCR 圖示與未問細節消失，不重花 PDF 費用。

## 2026-08-23（v29.6.267 / PDF 已有正確頁面卻被一筆錯型號證據拖垮）

- 正式 TestUI 第 3 題已正確延續 `S32FG502EC`、只掛 1 份正確 PDF，Gemini 2.5 Flash 實際找到第 33、34 頁；但同次輸出又帶入只適用 `S27FG502SC／S27FG706EC` 的第 35 頁 AI Mode，舊全有全無驗證將 2 筆有效證據一起丟掉。該輪 NT$0.2228，測試累計 NT$1.3643。
- 本機以 PDF 文字核對：第 33 頁是 Game 功能表，第 34 頁 Virtual Aim Point 明列適用 `S27FG502EC／S32FG502EC`，第 35 頁 AI Mode 不適用 `S32FG502EC`。因此根因是主張與 evidence 沒有一對一，不是 PDF 未讀或模型需要升級。
- Structured Output 改為每筆 evidence 自帶一項 `supportedAnswer`；normalizer 只組合逐筆通過頁碼、scope、目標型號、摘錄、數字與否定語意守門的答案。2/3 有效保留 2 筆，0 筆才失敗；頂層 answer／operationPath 完全忽略。
- 新增部分有效、同頁多主張、錯型號、摘錄外數字、無證據否定、矛盾 found、頂層錯答與 marker 不外露契約。來源路由契約通過；模型、thinking 與呼叫次數未增加。

## 2026-08-23（v29.6.266 / 真人追問抓到型號在授權前倒退）

- `後者的虛擬準心要從螢幕哪個選單打開？` 一度在畫面正確顯示 `S32FG502EC`，但終答保存時 `routingQuestion` 未同步持久型號，舊 `suggested_models=S27FG502EC` 又覆寫最近問題；點手冊後因此讀成 S27 手冊。這是通用狀態傳遞缺口，不是 PDF 或模型能力問題。
- 修正本輪 resolved model 同步；並將「哪個選單打開」等通用倒裝語序納入操作分類，使完整型號且 RULE 不足時直接進 PDF，不再多花一次 Fast 後只顯示 CTA。
- v29.6.265 真人重驗還命中 v29.6.264 的 10 分鐘來源快取，證明跨版本沿用會遮蔽正式修復。現在只允許同版本同題去重；新版本一律重新驗證證據。
- 此輪發現前實際累計約 NT$0.9272；錯型號 PDF NT$0.2020、Fast 中繼 NT$0.0123，累計約 NT$1.1415。模型與 thinking 未調高。

## 2026-08-23（v29.6.265 / 第 3 題真人測試抓到 PDF scope 契約衝突）

- `32 吋那款的虛擬準心要去哪裡開？` 已正確解析為 `S32FG502EC`，掛載 1 份正確 PDF、找到第 49 頁；但 schema 允許的「依型號而異」被 validator 當非法，官方答案遭丟棄後多花一次 Web，該輪 NT$0.2338。
- 修正為接受 `依型號而異` 的目前型號明文摘錄；錯型號仍失敗。混合共通／專屬 evidence 不得誤標全檔共通。模型、thinking、配額與呼叫數不變。

## 2026-08-23（v29.6.264 / 十題真人追問定位選型與比較結構缺口）

- 正式 TestUI 第一輪已問 10 個不同語意問題；實際付費生成約 NT$0.6934。失敗集中在系列候選被前 10 款截斷、普通新題誤接舊候選、自然比較漏掉上一款，以及比較函式只回固定欄位。這些是路由／資料狀態缺陷，不以升級模型掩蓋。
- 系列候選改為先掃完整規格資料再依首句描述縮選；手冊選單只列實際 PDF 覆蓋。Fast pending 的新功能題會結束舊候選，合法 `#型號:`、描述式縮選與延遲舊按鍵各有獨立守門，候選 TTL 統一 10 分鐘。
- 雙型號比較改為使用者指定欄位的完整 Evidence 契約，新增 30 分鐘比較追問狀態；前者／後者／唯一尺寸描述直接解析，真正含糊的「這台」才讓使用者選原兩款。
- PDF 身分改為使用者選定型號優先，避免共用手冊推定值蓋過選擇。Fast／PDF／Web 模型、thinking、配額與呼叫次數均未調高。
...
- 在 `buildDeterministicExactRuleReply_` 與 `QA.csv` 中精確載明 FM5 (M50F) 原廠底座為固定式且「無內建陀螺儀」，投影時直立手機兩側留黑邊，螢幕本身無法隨實體旋轉自動轉向；若需畫面隨實體旋轉滿版需選購具備陀螺儀與 HAS 旋轉支架的 M8/M9。
