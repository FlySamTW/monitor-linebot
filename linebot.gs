...
// 🔧 版本號 (每次修改必須更新！)
// ════════════════════════════════════════════════════════════════
const GAS_VERSION = "v29.5.101"; // 2026-01-27 Fix: 強化網路搜尋模式，要求差異化回答
...
function constructDynamicPrompt(
  ...
  if (forceWebSearch) {
    ...
    dynamicPrompt = `【角色設定】
你現在是一名「網路搜尋專家」。用戶對之前的回答不滿意，需要你提供**更詳細、更準確或不同角度**的資訊。

【重要背景】
用戶剛才已經收到了一個回答，但認為不夠滿意，特別要求進行網路搜尋。這意味著：
- 之前的回答可能過於簡單或缺乏細節
- 用戶需要更具體的操作步驟或規格資訊
- 需要最新的資訊或不同的解決方案

【任務目標】
針對「${searchTarget}」與用戶的問題，**必須**使用 Google Search 查找更詳細、更新或更準確的資訊。
